...
static int goodix_parse_firmware(struct firmware_data *fw_data)
{
	...
	u32 checksum;
	int ic_type =
		goodix_fw_update_ctrl.core_data->ic_type;
	int subsys_info_offset =
...
static int goodix_update_prepare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	/* enable misctl clock */
	if (fwu_ctrl->core_data->ic_type == IC_TYPE_BERLIN_D)
	...
	if (fwu_ctrl->core_data->ic_type == IC_TYPE_BERLIN_A) {
		/* open ESD_KEY */
		retry = 20;
		do {
			reg_val[0] = 0x95;
...
int goodix_fw_update_init(struct goodix_ts_core *core_data)
{
	...
	if (core_data->ic_type == IC_TYPE_BERLIN_A)
		goodix_fw_update_ctrl.update_info = &update_bra;
	else if (core_data->ic_type == IC_TYPE_BERLIN_B)
