...
static int goodix_power_ctrl(void *priv, bool on_off);
...
static void goodix_ts_procfs_init(struct goodix_ts_core *core_data)
{
	...
	if (core_data->ic_type == IC_TYPE_BERLIN_D) {
...
{
	int i;
	struct object_info obj_info = {.slots = 0,};

	for (i = 0; i < GOODIX_MAX_TOUCH; i++) {
		if (touch_data->coords[i].status == TS_TOUCH) {
			obj_info.slots |= (1 << i);
			obj_info.obj[i].x = touch_data->coords[i].x;
			obj_info.obj[i].y = touch_data->coords[i].y;
		} else {
			obj_info.slots &= ~(1 << i);
		}
	}

	hbp_request_or_report_unlocked(cd, HBP_REPORT_DATA, HBP_REPORT_OBJS, &obj_info, 0);
}
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	if (likely(!ret)) {
		...
		if (core_data->board_data.pen_enable &&
			...
			goodix_ts_report_pen(core_data,
					&ts_event->pen_data);
		}
...
static void goodix_ts_esd_work(struct work_struct *work)
{
	...
	if (ret) {
		...
		goodix_power_ctrl(cd, false);
		goodix_power_ctrl(cd, true);
	}
...
static void goodix_preinit(void *priv, int boot_mode) //
{

}

static void goodix_init(void *priv, int boot_mode)
{
}

static void goodix_reset(void *priv)
{
}

static int goodix_suspend(void *priv, struct hbp_feature *feature)
{

	return 0;
}

static int goodix_resume(void *priv, struct hbp_feature *feature)
{
	return 0;
}

static int goodix_early_resume(void *priv, struct hbp_feature *feature)
{
	return 0;
}

static int goodix_fw_update(void *priv, const struct firmware *entry, bool force)
{
	return 0;
}

static int goodix_get_chip_info(void *priv, struct chip_info *info)
{
	return 0;
}

static int goodix_get_raw(void *priv)
{
	return 0;
}

static int goodix_get_diff(void *priv)
{
	return 0;
}

static int goodix_get_base(void *priv)
{
	return 0;
}

static int goodix_send_cmd(void *priv, int cmd, struct param_value *param)
{
	return 0;
}
...
static int goodix_power_ctrl(void *priv, bool on_off)
{
	struct goodix_ts_core *cd = (struct goodix_ts_core *)priv;
	return cd->hw_ops->power_on(cd, on_off);
}

static struct common_operations goodix_ops = {
	.irq_fn = goodix_ts_threadirq_func,
	.pre_init = goodix_preinit,
	.init = goodix_init,
	.reset = goodix_reset,
	.suspend = goodix_suspend,
	.resume = goodix_resume,
	.early_resume = goodix_early_resume,
	.fw_update = goodix_fw_update,
	.get_chip_info = goodix_get_chip_info,
	.get_raw = goodix_get_raw,
	.get_diff = goodix_get_diff,
	.get_base = goodix_get_base,
	.send_cmd = goodix_send_cmd,
	.power_ctrl = goodix_power_ctrl,
};
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	core_data->ic_type = goodix_get_ic_type(pdev->dev.of_node);
	if (core_data->ic_type < 0) {
		ts_err("ic_type is invalid\n");
		return -EINVAL;
	}
	...
	ret = hbp_register_devices(core_data,
								&pdev->dev,
								&goodix_ops,
								"BOE",
								&core_data->bus_io);
	if (ret < 0) {
		hbp_err("failed to register devices\n");
		return -EINVAL;
	}
...
static struct of_device_id goodix_ts_of_match[] = {
	{ .compatible = GOODIX_CORE_DRIVER_NAME},
	{}
};
...
static struct platform_driver goodix_ts_driver = {
	.driver = {
		...
		.pm = &dev_pm_ops,
#endif
		.of_match_table = goodix_ts_of_match,
	},
