...
static int brl_select_spi_mode(struct goodix_ts_core *cd)
{
	...
	if (cd->bus_io->type == GOODIX_BUS_TYPE_I2C ||
			cd->ic_type != IC_TYPE_BERLIN_A)
		return 0;
...
static int brl_reset_after(struct goodix_ts_core *cd)
{
	...
	if (cd->ic_type != IC_TYPE_BERLIN_A)
		return 0;
...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (!on) {
		ret = hbp_request_or_report_unlocked(cd, HBP_REQUEST_SOURCE, HBP_POWER_RESET, NULL, on);
		if (ret < 0) {
			ts_err("failed to set reset off\n");
		}
	}

	ret = hbp_request_or_report_unlocked(cd, HBP_REQUEST_SOURCE, HBP_POWER_VDDI, NULL, on);
	if (ret < 0) {
		ts_err("failed to set vddi\n");
		goto exit;
	}
	usleep_range(3000, 3100);

	ret = hbp_request_or_report_unlocked(cd, HBP_REQUEST_SOURCE, HBP_POWER_AVDD, NULL, on);
	if (ret < 0) {
		ts_err("failed to set avdd\n");
		goto exit;
	}
...
static int brl_reset(struct goodix_ts_core *cd, int delay)
{
	...
	hbp_request_or_report_unlocked(cd, HBP_REQUEST_SOURCE, HBP_POWER_RESET, NULL, 0);
	usleep_range(2000, 2100);
	hbp_request_or_report_unlocked(cd, HBP_REQUEST_SOURCE, HBP_POWER_RESET, NULL, 1);
	if (delay < 20)
		...
		unsigned char *data, unsigned int len)
{
	int ret = 0;
	uint8_t tx_buf[9] = {0};
	struct hbp_bus_io *bus = cd->bus_io;
	...
	if (cd->ic_type == IC_TYPE_BERLIN_A) {
		ret = bus->read_block_onemsg(bus, tx_buf, 9, data, len);
	} else {
		ret = bus->read_block_onemsg(bus, tx_buf, 8, data, len);
	}
		 ...
		 unsigned char *data, unsigned int len)
{
	...
	ret = bus->addr_write_block(bus, tx_buf, 5, data, len);
