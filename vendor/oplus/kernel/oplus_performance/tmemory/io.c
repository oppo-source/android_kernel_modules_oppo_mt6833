...
static void detect_memleak(struct tmemory_device *tm)
{
	...
	tmemory_err(tm, "trans_slab: %d\ndiscard_slab: %d\nread_slab: %d\n"
		    ...
		    atomic_read(&tm->crypto_slab), atomic_read(&tm->bio_cnt),
...
{
	struct tmemory_device *tm;
	struct bio *origin_bio = bio->bi_private;
	unsigned long flags;
	bool wakeup = false;
#if LINUX_KERNEL_515
	tm = (struct tmemory_device *)origin_bio->bi_bdev->bd_disk->private_data;
#elif LINUX_KERNEL_414 || LINUX_KERNEL_419 || LINUX_KERNEL_504 || LINUX_KERNEL_510
	tm = (struct tmemory_device *)origin_bio->bi_disk->private_data;
#endif
	bio_put(bio);
	bio_endio(origin_bio);
...
#endif
{
	...
	if (state == TM_STATE_STOPPED || state == TM_STATE_STARTING) {
		struct bio *bypass_bio = bio_alloc(GFP_NOIO, 1);
		__bio_clone_fast(bypass_bio, bio);
		bio_crypt_clone(bypass_bio, bio, GFP_NOIO);

		bypass_bio->bi_private = bio;
		bypass_bio->bi_end_io = bypass_endio;
		bio_set_dev(bypass_bio, tm->rbd);

		do_submit_bio(bypass_bio);
