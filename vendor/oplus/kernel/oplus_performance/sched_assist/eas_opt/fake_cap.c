...
static int set_cluster_fake_cap(int cluster_idx, int fake_cap_multiple)
{
	...
	for_each_cpu(cpu, &cpus) {
		...
		rq = cpu_rq(cpu);
		rq->cpu_capacity_orig = mult_frac(fmax_capacity, fake_cap_multiple, 100);
...
static const struct file_operations proc_fake_cap_fops = {
	.write		= proc_fake_cap_write,
	.read		= proc_fake_cap_read,
};
