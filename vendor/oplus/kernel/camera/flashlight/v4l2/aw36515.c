...
#define AW36515_FLASH_POWER 3770
#define AW36515_TORCH_POWER 569
...
static int aw36515_select_led(struct aw36515_flash *flash,
										int led_num)
{
	int rval;
...
static int aw36515_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	flashlight_kicker_pbm_by_device_id(
				&flash->flash_dev_id[AW36515_LED0],
				AW36515_TORCH_POWER);
	mdelay(1);
#endif
...
static int aw36515_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case OPLUS_FLASH_IOC_SELECT_LED_NUM:
		if (fl_arg->arg == AW36515_CONTROL_LED0 || fl_arg->arg == AW36515_CONTROL_LED1) {
			aw36515_flash_data->led_mode = V4L2_FLASH_LED_MODE_FLASH;
			 ret = aw36515_select_led(aw36515_flash_data, fl_arg->arg);
			if (ret < 0) {
	...
	case FLASH_IOC_SET_SCENARIO:
		...
		if (scenario & FLASHLIGHT_SCENARIO_CAMERA_MASK) {
			flashlight_kicker_pbm_by_device_id(
				&aw36515_flash_data->flash_dev_id[AW36515_LED0],
				AW36515_FLASH_POWER);
		} else {
			flashlight_kicker_pbm_by_device_id(
				&aw36515_flash_data->flash_dev_id[AW36515_LED0],
