...
static unsigned int do_single_lsc_konkamain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_konkamain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_konkamain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_konkamain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x015E012C, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00004000, 0x0000074C, do_single_lsc_konkamain},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_konkamain}, //Start address, block size is useless
		{0x00000001, 0x00005300, 0x00001648, do_pdaf_konkamain},
		...
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_konkamain}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT konkamain_op_eeprom = {
	.name = "konkamain_op_eeprom",
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	#if READ_AVG /* AWBRG&BG integer And fractional parts */
	long long ReadAvgData= 0;
	#endif
	(void) start_addr;
	(void) block_size;
	...
	if (0x1 & AWBAFConfig) {
		...
		awb_offset = 0x0530;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 6, (unsigned char *) & ReadAvgData);
		if (read_data_size > 0)	{
			if (ReadAvgData != 0x0000000000000000 &&
				ReadAvgData != 0x0000FFFFFFFFFFFF) {
				pCamCalData->Single2A.S2aAwb.r1RG_avg_lnt = ReadAvgData & 0xFFFF;
				pCamCalData->Single2A.S2aAwb.r1RG_avg_dec = (ReadAvgData >> 16) & 0xFFFFFFFF;
			} else {
				pCamCalData->Single2A.S2aAwb.r1RG_avg_lnt = 0;
				pCamCalData->Single2A.S2aAwb.r1RG_avg_dec = 0;
			}
			printk("Read r1RG_avg OK 0x%llx %d.%d\n", ReadAvgData, pCamCalData->Single2A.S2aAwb.r1RG_avg_lnt, pCamCalData->Single2A.S2aAwb.r1RG_avg_dec);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read r1RG_avg Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		awb_offset = 0x0536;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		if (read_data_size > 0)	{
			if (ReadAvgData != 0x0000000000000000 &&
				ReadAvgData != 0x0000FFFFFFFFFFFF) {
				pCamCalData->Single2A.S2aAwb.r1BG_avg_lnt = ReadAvgData & 0xFFFF;
				pCamCalData->Single2A.S2aAwb.r1BG_avg_dec = (ReadAvgData >> 16) & 0xFFFFFFFF;
			} else {
				pCamCalData->Single2A.S2aAwb.r1BG_avg_lnt = 0;
				pCamCalData->Single2A.S2aAwb.r1BG_avg_dec = 0;
			}
			printk("Read r1BG_avg OK 0x%llx %d.%d\n", ReadAvgData, pCamCalData->Single2A.S2aAwb.r1BG_avg_lnt, pCamCalData->Single2A.S2aAwb.r1BG_avg_dec);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read r1BG_avg Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		awb_offset = 0x053C;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		if (read_data_size > 0)	{
			if (ReadAvgData != 0x0000000000000000 &&
				ReadAvgData != 0x0000FFFFFFFFFFFF) {
				pCamCalData->Single2A.S2aAwb.r1GoldenRG_lnt = ReadAvgData & 0xFFFF;
				pCamCalData->Single2A.S2aAwb.r1GoldenRG_dec = (ReadAvgData >> 16) & 0xFFFFFFFF;
			} else {
				pCamCalData->Single2A.S2aAwb.r1GoldenRG_lnt = 0;
				pCamCalData->Single2A.S2aAwb.r1GoldenRG_dec = 0;
			}
			printk("Read r1GoldenRG OK 0x%llx %d.%d\n", ReadAvgData, pCamCalData->Single2A.S2aAwb.r1GoldenRG_lnt, pCamCalData->Single2A.S2aAwb.r1GoldenRG_dec);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read r1GoldenRG Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		awb_offset = 0x0542;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		if (read_data_size > 0)	{
			if (ReadAvgData != 0x0000000000000000 &&
				ReadAvgData != 0x0000FFFFFFFFFFFF) {
				pCamCalData->Single2A.S2aAwb.r1GoldenBG_lnt = ReadAvgData & 0xFFFF;
				pCamCalData->Single2A.S2aAwb.r1GoldenBG_dec = (ReadAvgData >> 16) & 0xFFFFFFFF;
			} else {
				pCamCalData->Single2A.S2aAwb.r1GoldenBG_lnt = 0;
				pCamCalData->Single2A.S2aAwb.r1GoldenBG_dec = 0;
			}
			printk("Read r1GoldenBG_avg OK 0x%llx %d.%d\n", ReadAvgData, pCamCalData->Single2A.S2aAwb.r1GoldenBG_lnt, pCamCalData->Single2A.S2aAwb.r1GoldenBG_dec);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read r1GoldenBG_avg Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		awb_offset = 0x0550;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		if (read_data_size > 0)	{
			if (ReadAvgData != 0x0000000000000000 &&
				ReadAvgData != 0x0000FFFFFFFFFFFF) {
				pCamCalData->Single2A.S2aAwb.r2RG_avg_lnt = ReadAvgData & 0xFFFF;
				pCamCalData->Single2A.S2aAwb.r2RG_avg_dec = (ReadAvgData >> 16) & 0xFFFFFFFF;
			} else {
				pCamCalData->Single2A.S2aAwb.r2RG_avg_lnt = 0;
				pCamCalData->Single2A.S2aAwb.r2RG_avg_dec = 0;
			}
			printk("Read r2RG_avg OK 0x%llx %d.%d\n", ReadAvgData, pCamCalData->Single2A.S2aAwb.r2RG_avg_lnt, pCamCalData->Single2A.S2aAwb.r2RG_avg_dec);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read r2RG_avg Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		awb_offset = 0x0556;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		if (read_data_size > 0)	{
			if (ReadAvgData != 0x0000000000000000 &&
				ReadAvgData != 0x0000FFFFFFFFFFFF) {
				pCamCalData->Single2A.S2aAwb.r2BG_avg_lnt = ReadAvgData & 0xFFFF;
				pCamCalData->Single2A.S2aAwb.r2BG_avg_dec = (ReadAvgData >> 16) & 0xFFFFFFFF;
			} else {
				pCamCalData->Single2A.S2aAwb.r2BG_avg_lnt = 0;
				pCamCalData->Single2A.S2aAwb.r2BG_avg_dec = 0;
			}
			printk("Read r1RG_avg OK 0x%llx %d.%d\n", ReadAvgData, pCamCalData->Single2A.S2aAwb.r2BG_avg_lnt, pCamCalData->Single2A.S2aAwb.r2BG_avg_dec);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read r2BG_avg Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		awb_offset = 0x055C;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		if (read_data_size > 0)	{
			if (ReadAvgData != 0x0000000000000000 &&
				ReadAvgData != 0x0000FFFFFFFFFFFF) {
				pCamCalData->Single2A.S2aAwb.r2GoldenRG_lnt = ReadAvgData & 0xFFFF;
				pCamCalData->Single2A.S2aAwb.r2GoldenRG_dec = (ReadAvgData >> 16) & 0xFFFFFFFF;
			} else {
				pCamCalData->Single2A.S2aAwb.r2GoldenRG_lnt = 0;
				pCamCalData->Single2A.S2aAwb.r2GoldenRG_lnt = 0;
			}
			printk("Read r2GoldenRG_avg OK 0x%llx %d.%d\n", ReadAvgData, pCamCalData->Single2A.S2aAwb.r2GoldenRG_lnt, pCamCalData->Single2A.S2aAwb.r2GoldenRG_dec);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read r2GoldenRG_avg Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		awb_offset = 0x0562;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		if (read_data_size > 0)	{
			if (ReadAvgData != 0x0000000000000000 &&
				ReadAvgData != 0x0000FFFFFFFFFFFF) {
				pCamCalData->Single2A.S2aAwb.r2GoldenBG_lnt = ReadAvgData & 0xFFFF;
				pCamCalData->Single2A.S2aAwb.r2GoldenBG_dec = (ReadAvgData >> 16) & 0xFFFFFFFF;
			} else {
				pCamCalData->Single2A.S2aAwb.r2GoldenBG_lnt = 0;
				pCamCalData->Single2A.S2aAwb.r2GoldenBG_dec = 0;
			}
			printk("Read r2GoldenBG_avg OK 0x%llx %d.%d\n", ReadAvgData, pCamCalData->Single2A.S2aAwb.r2GoldenBG_lnt, pCamCalData->Single2A.S2aAwb.r2GoldenBG_dec);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read r2GoldenBG_avg Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		#endif
	}
