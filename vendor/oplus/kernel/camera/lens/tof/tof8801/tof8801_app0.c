  ...
  do {
    ...
    if (error == 0) {
      ...
      if ((error == 0) && TOF8801_STAT_CPU_READY(cpu_stat) && (state == OL_STATE_IDLE)) {
        return 0;
      } else if ((error == 0) && (cpu_stat == OL_STATE_STARTUP) && (state == OL_STATE_STARTUP)) {
        dev_err(&chip->client->dev,
                "Error App0 start up; cpu_stat: %#x state: %#x\n", cpu_stat, state);
        return -EIO;
      }
    }
...
static int tof8801_app0_check_osc(struct tof_sensor_chip *chip)
{
  ...
  if (chip->app0_app.clk_cr.trim_count >= chip->app0_app.clk_iterations) {
    ...
    if (ratioQ15 < TOF8801_APP0_MIN_FREQ_RATIO_Q15 && ratioQ15 > TOF8801_APP0_MAX_FREQ_RATIO_Q15) {
      dev_err(&chip->client->dev, "ratioQ15:%d error, skip trim", ratioQ15);
      return 0;
    }
...
void tof8801_app0_default_cap_settings(struct tof8801_app0_application *app0_app)
{
  ...
  // app0_app->int_cmpres.pers = (uint8_t)TOF8801_APP0_DIST_PERS_DEFAULT;
  // app0_app->int_cmpres.low_thres[0] = (uint8_t)(TOF8801_APP0_DIST_LOW_THRES_DEFAULT);
  // app0_app->int_cmpres.low_thres[1] = (uint8_t)(TOF8801_APP0_DIST_LOW_THRES_DEFAULT>>8);
  // app0_app->int_cmpres.high_thres[0] = (uint8_t)(TOF8801_APP0_DIST_HIGH_THRES_DEFAULT);
  // app0_app->int_cmpres.high_thres[1] = (uint8_t)(TOF8801_APP0_DIST_HIGH_THRES_DEFAULT>>8);
}
...
int tof8801_app0_perform_factory_calibration(void *tof_chip)
{
  ...
  if ((error == 0) && (chip->app0_app.diag_state_mask == 0)) {
...
void tof8801_app0_init_app(struct tof8801_app0_application *app0)
{
  ...
  /* read interrupt compression settings */
  // error = tof8801_app0_read_int_cmpres((void*)chip);
  // if (error) {
  //   dev_err(&chip->client->dev, "Error tof8801_app0_read_int_cmpres.\n");
  // }
}

int tof8801_histogram_enable(void *tof_chip, bool enable) {
  struct tof_sensor_chip *chip = (struct tof_sensor_chip *)tof_chip;
  // write parameters
  uint8_t buf[11] = {0};
  ...
  return tof_i2c_write(chip->client, TOF8801_APP0_CMD_START, buf, 11);
}
...
int tof8801_app0_capture(void *tof_chip, int capture)
{
  ...
  if (capture) {
    ...
    if (chip->histogram_enable) {
      dev_err(&chip->client->dev, "tof8801_histogram_enable");
      tof8801_histogram_enable(chip, 1);
    }
    ...
    chip->app0_app.cap_settings.buf[6] |= 0xC0;
    ...
    if (1) {
      ...
      if ((error == 0) && (chip->app0_app.cap_settings.period == 0) &&
        ...
        error = mod_timer(&chip->meas_timer,
                        jiffies + msecs_to_jiffies(MEASURE_TIMEOUT_MSEC));
        chip->interrupt_recv = 0;
    }
  } else {
    del_timer_sync(&chip->meas_timer); //delete measure timer
    // histogram disalbe
    if (chip->histogram_enable) {
      dev_err(&chip->client->dev, "tof8801_histogram_disable");
      tof8801_histogram_enable(chip, 0);
    }
    //stop measurement
...
int tof8801_app0_check_cmd_status(void *tof_chip, uint8_t cmd)
{
  struct tof_sensor_chip *chip = (struct tof_sensor_chip *)tof_chip;
  int error = 0;
  int idx = 0;
  ...
  while(idx < 10){
    ...
    //read command status regoster
    error = tof_i2c_read(chip->client, OL_PREVIOUS_OFFSET, &dat, 1);
    if (error) {
      dev_err(&chip->client->dev, "Error reading command status reg");
      tof8801_app0_report_error(chip, ERR_COMM, DEV_OK);
      return error;
    }
    if (dat == cmd){
  ...
  dev_err(&chip->client->dev, "Error tof8801_app0_check_cmd_status timeout");
  tof8801_app0_report_error(chip, ERR_COMM, DEV_OK);
  return -1;
}

// int tof8801_app0_write_int_cmpres(void *tof_chip)
// {
//   struct tof_sensor_chip *chip = (struct tof_sensor_chip *)tof_chip;
//   int error = 0;

//   chip->app0_app.int_cmpres.cmd = OL_COMMAND_WRITE_INT_CMPRES;

//   error = tof_i2c_write(chip->client, TOF8801_APP0_SET_INT_CMPRES_START,
//                           &chip->app0_app.int_cmpres.pers,
//                           sizeof(chip->app0_app.int_cmpres));
//   if (error) {
//       dev_err(&chip->client->dev, "Error writing interrupt compression command");
//       tof8801_app0_report_error(chip, ERR_COMM, DEV_OK);
//       return error;
//   }
//   error = tof8801_app0_check_cmd_status(tof_chip, OL_COMMAND_WRITE_INT_CMPRES);
//   if (error){
//       dev_err(&chip->client->dev, "Error write interrupt compression command failed");
//   }
//   return error;
// }

// int tof8801_app0_read_int_cmpres(void *tof_chip)
// {
//   struct tof_sensor_chip *chip = (struct tof_sensor_chip *)tof_chip;
//   uint8_t buf[7];
//   int error = 0;

//   error = tof8801_set_register(chip->client, OL_COMMAND_OFFSET, OL_COMMAND_READ_INT_CMPRES);
//   if (error) {
//       dev_err(&chip->client->dev, "Error reading interrupt compression command");
//       tof8801_app0_report_error(chip, ERR_COMM, DEV_OK);
//       return error;
//   }
//   error = tof8801_app0_check_cmd_status(tof_chip, OL_COMMAND_READ_INT_CMPRES);
//   if (error){
//     dev_err(&chip->client->dev, "Error read interrupt compression command failed");
//     return error;
//   }
//   //read config success
//   error = tof_i2c_read(chip->client, OL_REGISTER_CONTENTS_OFFSET, buf, sizeof(buf));
//   if (error) {
//     dev_err(&chip->client->dev, "Error read interrupt compression setting failed");
//     tof8801_app0_report_error(chip, ERR_COMM, DEV_OK);
//     return error;
//   }
//   if (buf[0] == OL_COMMAND_READ_INT_CMPRES){
//     memcpy(&chip->app0_app.int_cmpres.pers, &buf[2], sizeof(chip->app0_app.int_cmpres));
//     return 0;
//   }
//   dev_err(&chip->client->dev, "Error read interrupt compression setting bad data");
//   tof8801_app0_report_error(chip, ERR_COMM, DEV_OK);
//   return -1;
// }
...
void tof8801_app0_measure_timer_expiry_callback(struct timer_list *t)
{
  ...
  if (1) {
    ...
    if (chip->driver_debug){
        dev_info(&chip->client->dev, "timer callback called");
    }
    if (!chip->interrupt_recv){
      //when interrupt compression enabled, when timeout, reset the clock correction variable
      TMF8801_cr_init(&chip->app0_app.clk_cr);
      if (chip->driver_debug){
        dev_info(&chip->client->dev, "TMF8801_cr_init called inside timer callback ");
      }
    }
    mod_timer(&chip->meas_timer,
                        jiffies + msecs_to_jiffies(MEASURE_TIMEOUT_MSEC));
    chip->interrupt_recv = 0;
  }
}
...
int tof8801_app0_clock_skew_correct_results(struct tof_sensor_chip *chip)
{
  ...
  /* clock trim is only working when object is detected */
...
int tof8801_app0_read_results(struct tof_sensor_chip *chip)
{
  ...
  if (chip->driver_debug) {
    dev_info(&chip->client->dev, "ams-tof capture result: distance= %d (mm), confidence= %d/63, inc result cnt= %d",
  ...
  chip->confidence = (first_obj_dis >> 16) & 0x3f;
  chip->result_cnt = (first_obj_dis >> 24) & 0xff;
                              ...
                              tof_core1_diag_info_t diag_info)
{
  ...
  if (tdc == tdcAvailAll5) {
    ...
    // copy app0->hist_frame.buf to histogram bins buf
    // hist_frame: Identifier(1 Byte)+SizeLSB (1 Byte)+SizeMSB (1 Byte)+256
    if (chip->histogram_enable) {
      spin_lock(&chip->data_lock);
      char *buf = app0->hist_frame.buf + 3;
      for (int i = 0; i < TOF_HISTOGRAM_SUM; i++) {
        app0->histogram[i] = buf[i*2] | buf[i*2+1] << 8; //16bit per bin
      }
      spin_unlock(&chip->data_lock);
      // just print 0~32 bin data for test(total 128 bins)
      if (chip->driver_debug == 2) {
        for (int i = 0; i < 128; i++) {
          dev_info(&chip->client->dev, "bin[%d]:%d", i, app0->histogram[i]);
        }
      }
    }
    //place one full histogram frame into fifo
...
void tof8801_app0_process_irq(void *tof_chip, char int_stat)
{
  ...
  if (int_stat & IRQ_DIAG) {
    ...
    if (chip->driver_debug) {
      ...
      dev_info(&chip->client->dev,
               "IRQ: STATE: %#x diagNum: %#hhx diagHistSize: %d tdcAvail: %#hhx",
