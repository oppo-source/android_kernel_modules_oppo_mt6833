...
static void add_full(struct kmem_cache *s, struct kmem_cache_node *n, struct slab *slab);
static int slab_pad_check(struct kmem_cache *s, struct slab *slab);
static int check_object(struct kmem_cache *s, struct slab *slab, void *object, u8 val);
static inline void dec_slabs_node(struct kmem_cache *s, int node, int objects);
static void kd_deactivate_slab(struct kmem_cache *s, struct slab *slab,
...
static void save_track_hash_hook(void *data, bool alloc, struct track *p)
{
	...
	nr_entries = stack_depot_fetch(p->handle, &entries);
	/* save the stack hash */
	hash = jhash2((u32 *)entries,
		...
		struct kmem_cache **s)
{
	...
	cache_s = (struct kmem_cache *)atomic64_read(
			&kmalloc_debug_caches[kmalloc_type(flags, _RET_IP_)][index]);
	if (cache_s)
		*s = cache_s;
}
		...
		void *freelist, struct kmem_cache_cpu *c)
{
	...
	while (freelist && (nextfree = get_freepointer(s, freelist))) {
		 ...
		 */
		if (freelist_corrupted(s, slab, &freelist, nextfree))
			break;
...
static inline void kd_flush_slab(struct kmem_cache *s, struct kmem_cache_cpu *c)
{
	...
	kd_deactivate_slab(s, c->slab, c->freelist, c);
 ...
 */
static unsigned long *kd_get_map(struct kmem_cache *s, struct slab *slab)
{
	void *p;
	...
	bitmap_zero(kd_object_map, slab->objects);
...
static void print_slab_info(const struct slab *slab)
{
	...
	pr_err("INFO: Slab 0x%p objects=%u used=%u fp=0x%p flags=flags=%pGp\n",
		...
		const struct track *track)
{
	...
	nr_entries = stack_depot_fetch(track->handle, &entries);
#ifdef COMPACT_OPLUS_SLUB_TRACK
	 {
		int i;
		for (i = 0; i < l->depth; i++)
			l->addrs[i] = entries[i] + MODULES_VADDR;
	}
#else
	memcpy(l->addrs, entries, sizeof(l->addrs[0])*l->depth);
#endif
	return 0;
}
		...
		int buff_len, enum track_item alloc)
{
	...
	for_each_kmem_cache_node(s, node, n) {
		...
		list_for_each_entry(slab, &n->partial, slab_list) {
			ret = kd_process_slab(&t, s, slab, alloc);
			if (ret)
				dropped += ret;
		}
		...
		char *dump_buff, int len, enum track_item alloc)
{
	...
	for_each_kmem_cache_node(s, node, n) {
		...
		spin_lock_irqsave(&n->list_lock, flags);
		list_for_each_entry(slab, &n->partial, slab_list)
			kd_process_slab(&t, s, slab, alloc);
		list_for_each_entry(slab, &n->full, slab_list)
			kd_process_slab(&t, s, slab, alloc);
		spin_unlock_irqrestore(&n->list_lock, flags);
	}
