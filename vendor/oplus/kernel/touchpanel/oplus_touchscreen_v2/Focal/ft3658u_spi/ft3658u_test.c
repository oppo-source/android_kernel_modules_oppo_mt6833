...
static int fts_test_bus_read(u8 *cmd, int cmdlen, u8 *data, int datalen)
{
	...
	ret = fts_read((char *)write_buf, cmdlen,
...
static void ft3658u_autotest_populate_result_head(struct chip_data_ft3658u *ts_data,
...
#ifdef FTS_KIT
static int fts_get_threshold(struct chip_data_ft3658u *ts_data, struct auto_testdata *p_focal_testdata)
{
	int i = 0;
	int tx_num = ts_data->hw_res->tx_num;
	int rx_num = ts_data->hw_res->rx_num;
...
#else
static int fts_get_threshold_from_img(struct chip_data_ft3658u *ts_data, struct auto_testdata *p_focal_testdata)
{
	int ret = 0;
	int i = 0;
	int item_cnt = 0;
