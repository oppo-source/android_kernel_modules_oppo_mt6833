                                            ...
                                            size_t len, loff_t *pos)
{
    ...
    if ((buf_tmp[0] == 'r' || buf_tmp[0] == 'w') && buf_tmp[1] == ':' && buf_tmp[2] == 'x' && len > 3) {
        memset(w_data, 0x00, sizeof(w_data));
        //memcpy(p_tok_data, buf_tmp + 3, strlen(buf_tmp) - 3);
        scnprintf(p_tok_data, len - 3, "%s", buf_tmp + 3);

        token = strsep(&p_tok_data, delim);
        while (token != NULL) {
                                            ...
                                            size_t len, loff_t *pos)
{
    ...
    if ((buf_tmp[0] == 'r' || buf_tmp[0] == 'w') && buf_tmp[1] == ':' && buf_tmp[2] == 'x' && len > 2) {
        memset(w_data, 0x00, sizeof(w_data));
        //memcpy(p_tok_data, buf_tmp + 2, strlen(buf_tmp) - 2);
        scnprintf(p_tok_data, len - 2, "%s", buf_tmp + 2);
                                 ...
                                 size_t len, loff_t *pos)
{
    ...
    if (buf_tmp[0] == 'v') {
        ...
        ret = g_module_fp.fp_0f_upgrade_fw(fileName, NULL);
        pjadard_ts_data->power_on_upgrade = true;
...
int jadard_debug_init(void)
{
	...
	rdata_size = pjadard_ic_data->JD_X_NUM * pjadard_ic_data->JD_Y_NUM * buf_rd_byte_num;
	jd_buf = kzalloc(rdata_size * sizeof(uint8_t), GFP_KERNEL);
