...
struct jadard_module_fp {
    ...
    void (*fp_set_smooth_level)(uint8_t level);
    void (*fp_set_sensitive_level)(uint8_t level);
    void (*fp_set_esd_handle_support)(uint8_t status);
    void (*fp_set_report_rate_white_list_support)(uint8_t status);
    ...
    int (*fp_0f_upgrade_fw)(char *file_name, const struct firmware *oplus_fw);
#ifdef JD_ESD_CHECK
    int (*fp_0f_esd_upgrade_fw)(char *file_name, const struct firmware *oplus_fw);
    int (*fp_esd_ram_write)(uint32_t start_addr, uint8_t *wdata, uint32_t wlen);
#endif
    void (*fp_0f_operation)(struct work_struct *work);
#endif

    int (*fp_sorting_test)(char *json_buf, size_t len, struct seq_file *s, struct auto_testdata *jadard_testdata);
