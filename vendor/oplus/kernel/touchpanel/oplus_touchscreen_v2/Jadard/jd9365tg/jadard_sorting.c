...
static int jd_test_data_pop_out(char *rslt_buf, char *filepath, int result, struct auto_testdata *jadard_testdata)
{
#ifdef JD_OPLUS_TP
    ...
    if (jadard_testdata->fp) {
        JD_I("jadard_testdata->length = 0x%x\n", (uint32_t)jadard_testdata->length);
        if (jadard_testdata->length > g_rslt_data_len) {
            tp_test_write(jadard_testdata->fp, jadard_testdata->length, rslt_buf, g_rslt_data_len, jadard_testdata->pos);
        } else  {
		    JD_E("jadard_testdata->length < rslt_buf_len\n");
             ret_val = -EIO;
        }
	} else {
        JD_E("Write ITO result data to auto_testdata fail\n");
        ret_val = -EIO;
    }
...
static void jd_test_data_get(uint8_t *RAW, char *start_log, char *result, char *cnt_log, int now_item)
{
    ...
    memcpy(&jd_g_rslt_data[g_rslt_data_len], testdata, len/*SZ_SIZE*/);
    g_rslt_data_len += len;

    /* dbg */
    /*for(i = 0; i < SZ_SIZE; i++)
    {
        JD_I("0x%04X, ", jd_g_rslt_data[i + (now_item * SZ_SIZE)]);
        if(i > 0 && (i % 16 == 15))
            printk("\n");
    }*/
...
static uint32_t jadard_Item_Test_Function(uint8_t checktype, struct auto_testdata *jadard_testdata)
{
    int i;
    int raw_data;
    uint32_t index = 0;
    ...
    if (jadard_testdata) {
        if (checktype == JD_SORTING_OPEN_CHECK) {
            p_test_item_info = get_test_item_info(jadard_testdata->fw, 1);
        } else if (checktype == JD_SORTING_SHORT_CHECK) {
            p_test_item_info = get_test_item_info(jadard_testdata->fw, 2);
        } else if (checktype == JD_SORTING_NORMALACTIVE_NOISE) {
            p_test_item_info = get_test_item_info(jadard_testdata->fw, 3);
        } else {
            JD_E("Not support item: %s\n", jd_action_item_name[checktype - JD_SORTING_OPEN_CHECK]);
            kfree(rslt_log);
            kfree(start_log);
            kfree(cnt_log);
            return JD_MEM_ALLOC_FAIL;
        ...
        if (!p_test_item_info) {
            JD_I("item: %s get_test_item_info fail\n", jd_action_item_name[checktype - JD_SORTING_OPEN_CHECK]);
            kfree(rslt_log);
            kfree(start_log);
            kfree(cnt_log);
            return JD_MEM_ALLOC_FAIL;
        ...
        max_limits = (uint32_t *)(jadard_testdata->fw->data + p_test_item_info->top_limit_offset);
	    min_limits = (uint32_t *)(jadard_testdata->fw->data + p_test_item_info->floor_limit_offset);
        ...
        case JD_SORTING_OPEN_CHECK:
            for (i = 0; i < (pjadard_ic_data->JD_Y_NUM * pjadard_ic_data->JD_X_NUM); i++) {
                ...
                if ((raw_data < min_limits[i]) || (raw_data > max_limits[i])) {
                    if (fail_data_cnt == 0) {
...
static uint32_t jadard_TestItem_Select(uint8_t sorting_item, bool retry_flag, struct auto_testdata *jadard_testdata)
{
    SocReset_Flag = false;
    ...
    case JD_SORTING_OPEN_CHECK:
        ...
        if (SocReset_Flag == true) {
            if (retry_flag == false) {
                return jadard_TestItem_Select(sorting_item, true, jadard_testdata);
            } else {
                Soc_Debug_Number++;
        ...
        if (jadard_Item_Test_Function(JD_SORTING_OPEN_CHECK, jadard_testdata) != JD_SORTING_OK)
    ...
    case JD_SORTING_SHORT_CHECK:
        ...
        if (jadard_Item_Test_Function(JD_SORTING_SHORT_CHECK, jadard_testdata) != JD_SORTING_OK)
    ...
    case JD_SORTING_NORMALACTIVE_SB_DEV:
        ...
        if (jadard_Item_Test_Function(JD_SORTING_NORMALACTIVE_SB_DEV, jadard_testdata) != JD_SORTING_OK)
    ...
    case JD_SORTING_NORMALACTIVE_NOISE:
        ...
        if (jadard_Item_Test_Function(JD_SORTING_NORMALACTIVE_NOISE, jadard_testdata) != JD_SORTING_OK)
    ...
    case JD_SORTING_NORMALIDLE_RAWDATA_CHECK:
        ...
        if (jadard_Item_Test_Function(JD_SORTING_NORMALIDLE_RAWDATA_CHECK, jadard_testdata) != JD_SORTING_OK)
    ...
    case JD_SORTING_NORMALIDLE_NOISE:
        ...
        if (jadard_Item_Test_Function(JD_SORTING_NORMALIDLE_NOISE, jadard_testdata) != JD_SORTING_OK)
    ...
    case JD_SORTING_LPWUGACTIVE_SB_DEV:
        ...
        if (jadard_Item_Test_Function(JD_SORTING_LPWUGACTIVE_SB_DEV, jadard_testdata) != JD_SORTING_OK)
    ...
    case JD_SORTING_LPWUGACTIVE_NOISE:
        ...
        if (jadard_Item_Test_Function(JD_SORTING_LPWUGACTIVE_NOISE, jadard_testdata) != JD_SORTING_OK)
    ...
    case JD_SORTING_LPWUGIDLE_RAWDATA_CHECK:
        ...
        if (jadard_Item_Test_Function(JD_SORTING_LPWUGIDLE_RAWDATA_CHECK, jadard_testdata) != JD_SORTING_OK)
    ...
    case JD_SORTING_LPWUGIDLE_NOISE:
        ...
        if (jadard_Item_Test_Function(JD_SORTING_LPWUGIDLE_NOISE, jadard_testdata) != JD_SORTING_OK)
...
static int jadard_read_sorting_threshold(struct seq_file *s, struct auto_testdata *jadard_testdata)
{
    ...
    if ((s == NULL) && (jadard_testdata == NULL)) {
...
static int jadard_sorting_test(char *json_buf, size_t len, struct seq_file *s, struct auto_testdata *jadard_testdata)
{
    ...
    fw_ret = g_module_fp.fp_0f_upgrade_fw(fileName, NULL);
    if (fw_ret < 0) {
    ...
    ret = jadard_read_sorting_threshold(s, jadard_testdata);
    ...
    ret += jadard_TestItem_Select(JD_SORTING_SLEEP_IN, false, jadard_testdata);
    JD_I("JD_SORTING_SLEEP_IN: End %d\n\n", ret);

    JD_I("[JD_SORTING_SLEEP_OUT]\n");
    ret += jadard_TestItem_Select(JD_SORTING_SLEEP_OUT, false, jadard_testdata);
    ...
    if (jd_pst_global_variable[0][2] == 1) {
        ...
        ret += jadard_TestItem_Select(JD_SORTING_OPEN_CHECK, false, jadard_testdata);
    ...
    if (jd_pst_global_variable[1][2] == 1) {
        ...
        ret += jadard_TestItem_Select(JD_SORTING_SHORT_CHECK, false, jadard_testdata);
    ...
    if (jd_pst_global_variable[2][2] == 1) {
        ...
        ret += jadard_TestItem_Select(JD_SORTING_NORMALACTIVE_SB_DEV, false, jadard_testdata);
    ...
    if (jd_pst_global_variable[3][2] == 1) {
        ...
        ret += jadard_TestItem_Select(JD_SORTING_NORMALACTIVE_NOISE, false, jadard_testdata);
    ...
    if (jd_pst_global_variable[4][2] == 1) {
        ...
        ret += jadard_TestItem_Select(JD_SORTING_NORMALIDLE_RAWDATA_CHECK, false, jadard_testdata);
    ...
    if (jd_pst_global_variable[5][2] == 1) {
        ...
        ret += jadard_TestItem_Select(JD_SORTING_NORMALIDLE_NOISE, false, jadard_testdata);
    ...
    if (jd_pst_global_variable[6][2] == 1) {
        ...
        ret += jadard_TestItem_Select(JD_SORTING_LPWUGACTIVE_SB_DEV, false, jadard_testdata);
    ...
    if (jd_pst_global_variable[7][2] == 1) {
        ...
        ret += jadard_TestItem_Select(JD_SORTING_LPWUGACTIVE_NOISE, false, jadard_testdata);
    ...
    if (jd_pst_global_variable[8][2] == 1) {
        ...
        ret += jadard_TestItem_Select(JD_SORTING_LPWUGIDLE_RAWDATA_CHECK, false, jadard_testdata);
    ...
    if (jd_pst_global_variable[9][2] == 1) {
        ...
        ret += jadard_TestItem_Select(JD_SORTING_LPWUGIDLE_NOISE, false, jadard_testdata);
        JD_I("JD_SORTING_LPWUGIDLE_NOISE: End %d\n\n", ret);
    }
#endif

    jd_test_data_pop_out(jd_g_rslt_data, jd_g_file_path, ret , jadard_testdata);
