...
static int tp_control_cs_gpio(bool enable, unsigned int tp_index)
{
	...
	if (gpio_is_valid(ts->hw_res.cs_gpio)) {
		...
		TPD_INFO("%s:set cs %d\n", __func__, enable);
	} else if (!IS_ERR_OR_NULL(ts->hw_res.pin_set_high) && enable) {
		TPD_INFO("%s: going to set cs on spi mode.\n", __func__);
		pinctrl_select_state(ts->hw_res.pinctrl, ts->hw_res.pin_set_high);
	} else if (!IS_ERR_OR_NULL(ts->hw_res.pin_set_low) && !enable) {
