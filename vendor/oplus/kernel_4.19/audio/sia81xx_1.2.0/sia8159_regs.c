...
// add for sia8157
// Need to remove sia8157 register address {0x6A, 0x6F} from chip_id_ranges, and use the device tree for decoupling here
static const SIA_CHIP_ID_RANGE chip_id_ranges_for_sia8157[] = {
        {0x58, 0x58},
        {0x60, 0x68}
};
...
// add for sia8157
	...
	if(1 == en_spk_sia8157) {
		for (i = 0; i < ARRAY_SIZE(chip_id_ranges_for_sia8157); i++) {
			if (val >= chip_id_ranges_for_sia8157[i].begin && val <= chip_id_ranges_for_sia8157[i].end)
				return 0;
		}
		return -1;
	}
