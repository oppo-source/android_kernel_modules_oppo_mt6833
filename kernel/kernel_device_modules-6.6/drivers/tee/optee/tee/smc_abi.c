			     ...
			     struct optee_call_ctx *call_ctx)
{
	...
	case OPTEE_SMC_RPC_FUNC_CMD:
		if (rpc_arg) {
			...
			if (IS_ERR(arg)) {
				pr_notice("%s: tee_shm_get_va %p failed\n",
				       __func__, shm);
				break;
			}
		}
		...
		break;
	default:
		pr_notice("Unknown RPC func 0x%x\n",
...
optee_config_shm_memremap(optee_invoke_fn *invoke_fn, void **memremaped_shm)
{
	...
	if (res.result.status != OPTEE_SMC_RETURN_OK) {
		pr_notice("static shm service not available\n");
		return ERR_PTR(-ENOENT);
	}

	if (res.result.settings != OPTEE_SMC_SHM_CACHED) {
		pr_notice("only normal cached shared memory supported\n");
	...
	if (!va) {
		pr_notice("shared memory ioremap failed\n");
		return ERR_PTR(-EINVAL);
	}
...
static u32 i4_optee_probe_flag; //flag avoid repeat probe
...
static int optee_probe(struct platform_device *pdev)
{
	...
	if (i4_optee_probe_flag > 0) {
		pr_info("initialized driver already done, no need run again\n");
		return 0;
	}
	...
	if (!optee_msg_api_revision_is_compatible(invoke_fn)) {
		pr_notice("api revision mismatch\n");
		return -EINVAL;
	}
	...
	if (!optee_msg_exchange_capabilities(invoke_fn, &sec_caps,
					     &max_notif_value,
					     &rpc_param_count)) {
		pr_notice("capabilities mismatch\n");
		return -EINVAL;
	}
...
//		goto err_close_ctx;
	...
	if (sec_caps & OPTEE_SMC_SEC_CAP_ASYNC_NOTIF) {
		...
		if (rc < 0) {
			pr_notice("platform_get_irq: ret %d\n", rc);
	...
	optee_clkctrl_init(pdev->dev.of_node);
#endif

#if IS_ENABLED(CONFIG_OPTEE_REE_CONSOLE)
	rc = optee_kreeconsole_init();
	if (rc)
		pr_notice("kreeconsole init failed: %d\n", rc);
#endif
