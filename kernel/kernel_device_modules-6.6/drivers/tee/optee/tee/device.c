		       ...
		       u32 func)
{
	...
	if ((ret < 0) || ((inv_arg.ret != TEEC_SUCCESS) &&
		...
		pr_notice("PTA_CMD_GET_DEVICES invoke function err: %x\n",
		       inv_arg.ret);
		return -EINVAL;
	}
...
static int optee_register_device(const uuid_t *device_uuid)
{
	...
	if (rc) {
		pr_notice("device registration failed, err: %d\n", rc);
...
static int __optee_enumerate_devices(u32 func)
{
	...
	if (IS_ERR(device_shm)) {
		pr_notice("tee_shm_alloc_kernel_buf failed\n");
	...
	if (IS_ERR(device_uuid)) {
		pr_notice("tee_shm_get_va failed\n");
