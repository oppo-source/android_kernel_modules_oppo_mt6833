				    ...
				    struct tee_shm *shm)
{
	...
	if (rc)
		pr_notice("Unregister SHM id 0x%llx rc %d\n", global_handle, rc);

	rc = mem_ops->memory_reclaim(global_handle, 0);
	if (rc)
		pr_notice("mem_reclaim: 0x%llx %d", global_handle, rc);
					...
					const struct ffa_ops *ops)
{
	...
	rc = msg_ops->sync_send_receive(ffa_dev, &data);
	if (rc) {
		pr_notice("Unexpected error %d\n", rc);
		return false;
	}
	if (data.data0 != OPTEE_FFA_VERSION_MAJOR ||
	    data.data1 < OPTEE_FFA_VERSION_MINOR) {
		pr_notice("Incompatible OP-TEE API version %lu.%lu",
				    ...
				    unsigned int *rpc_param_count)
{
	...
	if (rc) {
		pr_notice("Unexpected error %d", rc);
		return false;
	}
	if (data.data0) {
		pr_notice("Unexpected exchange error %lu", data.data0);
		return false;
	}
