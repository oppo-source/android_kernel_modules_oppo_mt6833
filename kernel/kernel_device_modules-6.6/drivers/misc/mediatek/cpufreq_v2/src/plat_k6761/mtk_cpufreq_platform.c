...
__weak int sync_dcm_set_mp0_freq(unsigned int mhz)
{
	return 0;
}

__weak int sync_dcm_set_mp1_freq(unsigned int mhz)
{
	return 0;
}

__weak int sync_dcm_set_mp2_freq(unsigned int mhz)
{
	return 0;
}

__weak int sync_dcm_set_cci_freq(unsigned int mhz)
{
	return 0;
}
...
{
	regulator_proc1 = devm_regulator_get_optional(&pdev->dev, "vproc");
	if (GEN_DB_ON(IS_ERR(regulator_proc1), "vproc Get Failed"))
		return -ENODEV;

	regulator_sram1 = devm_regulator_get_optional(&pdev->dev, "vsram_proc");
