...
//    int k = 0;
    ...
    if (scenario_id == MSDK_SCENARIO_ID_CUSTOM1) {
        if (imgsensor.frame_length && imgsensor_info.custom6.linelength) {
            divisor = imgsensor_info.custom6.pclk / imgsensor.frame_length / imgsensor_info.custom6.linelength;
        }
    } else if (scenario_id == MSDK_SCENARIO_ID_CUSTOM6) {
        if (imgsensor.frame_length && imgsensor_info.custom1.linelength) {
            divisor = imgsensor_info.custom1.pclk / imgsensor.frame_length / imgsensor_info.custom1.linelength;
        }
    }
