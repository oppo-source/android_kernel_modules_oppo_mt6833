...
int get_io_reg_base(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6853-lvts");
	WARN_ON_ONCE(node == 0);
	if (node) {
		/* Setup IO addresses */
	...
	if (!thermal_irq_number) {
		...
		tscpu_dprintk("[THERM_CTRL] get irqnr failed=%d\n",
				thermal_irq_number);
		return 0;
	}
	...
	tscpu_dprintk("[THERM_CTRL] mcu_irq_num = %d\n",
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6853-infracfg_ao");
	WARN_ON_ONCE(node == 0);
	if (node) {
		/* Setup IO addresses */
