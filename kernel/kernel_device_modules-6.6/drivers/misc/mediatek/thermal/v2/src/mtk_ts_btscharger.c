...
#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
struct iio_channel *thermistor_ch2_bts;
#endif
...
#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
	ret = iio_read_channel_processed(thermistor_ch2_bts, &val);
	if (ret < 0) {
...
int mtktscharger_register_cooler_MT6877(void)
{
...
void mtktscharger_unregister_cooler_MT6877(void)
{
	if (cl_dev_sysrst) {
...
void mtkts_btscharger_prepare_table_MT6877(int table_num)
{
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		mtkts_btscharger_prepare_table_MT6877(g_RAP_ntc_table);
...
static int mtktscharger_pdrv_probe(struct platform_device *pdev)
{
	...
	thermistor_ch2_bts = devm_kzalloc(&pdev->dev, sizeof(*thermistor_ch2_bts),
		GFP_KERNEL);
	if (!thermistor_ch2_bts)
		return -ENOMEM;


	thermistor_ch2_bts = iio_channel_get(&pdev->dev, "thermistor-ch3");
	ret = IS_ERR(thermistor_ch2_bts);
	if (ret) {
		mtktscharger_dprintk_always(
...
err_unreg:
	mtktscharger_unregister_cooler_MT6877();
...
const struct of_device_id mt_thermistor_of_match3_bts[2] = {
	{.compatible = "mediatek,mtboard-thermistor6_tacoo",},
	{},
};
#endif

#define THERMAL_THERMISTOR_NAME    "mtboard-thermistor6_tacoo"
...
#if IS_ENABLED(CONFIG_OF)
		.of_match_table = mt_thermistor_of_match3_bts,
#endif
	},
};
...
int  mtk_tscharger_init(void)
{
	...
	mtkts_btscharger_prepare_table_MT6877(g_RAP_ntc_table);
	err = mtktscharger_register_cooler_MT6877();
	if (err)
		return err;
