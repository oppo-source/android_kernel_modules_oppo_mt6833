...
static int register_thermal_ipi(void)
{
	...
	ret = mtk_ipi_register(get_mcupm_ipidev(), CH_S_PLATFORM, NULL, NULL,
		(void *)&ack_data);
	if (ret != 0) {
	...
	unsigned int cmd, struct thermal_ipi_data *thermal_data)
{
	...
	case THERMAL_IPI_INIT_GRP6:
		thermal_data->cmd = cmd;
		ret = mtk_ipi_send_compl(get_mcupm_ipidev(), CH_S_PLATFORM,
...
{
	platform_driver_register(&thermal_ipi_driver);
	return 0;
}
void mtk_thermal_ipi_exit(void)
{
	platform_driver_unregister(&thermal_ipi_driver);
}
