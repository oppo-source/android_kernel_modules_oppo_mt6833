	...
	VL53LX_Error     *pcal_status)
{
	...
	struct VL53LX_refspadchar_config_t *prefspadchar  = &(pdev->refspadchar);
...
VL53LX_Error VL53LX_get_and_avg_xtalk_samples(
		...
		struct VL53LX_xtalk_range_results_t *pXR,
		...
		struct VL53LX_histogram_bin_data_t  *pavg_histo)
{
	...
	struct VL53LX_range_results_t      *prs =
			(struct VL53LX_range_results_t *) pdev->wArea1;

	struct VL53LX_range_data_t         *prange_data;
	struct VL53LX_xtalk_range_data_t   *pxtalk_range_data;
	...
	memcpy(pavg_histo, &(pdev->hist_data),
			sizeof(struct VL53LX_histogram_bin_data_t));
	...
	struct VL53LX_xtalk_histogram_shape_t     *pxtalk_shape)
{
	...
	VL53LX_Error  status = VL53LX_ERROR_NONE;
	struct VL53LX_xtalk_calibration_results_t *pX = pxtalk_cal;
	...
	VL53LX_Error                       *pcal_status)
{
	...
	VL53LX_Error status = VL53LX_ERROR_NONE;
	struct VL53LX_LLDriverData_t *pdev = VL53LXDevStructGetLLDriverHandle(Dev);
	struct VL53LX_xtalkextract_config_t *pX = &(pdev->xtalk_extract_cfg);
	struct VL53LX_xtalk_config_t *pC = &(pdev->xtalk_cfg);
	struct VL53LX_xtalk_calibration_results_t *pXC = &(pdev->xtalk_cal);
