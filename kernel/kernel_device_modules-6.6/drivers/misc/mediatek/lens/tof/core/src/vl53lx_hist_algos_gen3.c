	...
	struct VL53LX_hist_gen3_algo_private_data_t  *palgo)
{
	...
	struct VL53LX_hist_pulse_data_t *pdata;
	...
	struct VL53LX_hist_gen3_algo_private_data_t  *palgo)
{
	...
	struct VL53LX_hist_pulse_data_t  tmp;
	struct VL53LX_hist_pulse_data_t *ptmp = &tmp;
	struct VL53LX_hist_pulse_data_t *p0;
	struct VL53LX_hist_pulse_data_t *p1;
		...
		for (i = 1; i < palgo->VL53LX_p_046; i++) {
			...
			== VL53LX_HIST_TARGET_ORDER__STRONGEST_FIRST) {
				...
				if (p0->VL53LX_p_010 <
					...
					memcpy(ptmp,
					p1, sizeof(struct VL53LX_hist_pulse_data_t));
					memcpy(p1,
					p0, sizeof(struct VL53LX_hist_pulse_data_t));
					memcpy(p0,
					ptmp, sizeof(struct VL53LX_hist_pulse_data_t));
	...
	struct VL53LX_hist_gen3_algo_private_data_t  *palgo)
{
	...
	struct VL53LX_hist_pulse_data_t *pdata = &(palgo->VL53LX_p_003[pulse_no]);
	...
	struct VL53LX_histogram_bin_data_t           *ppulse)
{
	...
	memcpy(ppulse, pbins, sizeof(struct VL53LX_histogram_bin_data_t));
