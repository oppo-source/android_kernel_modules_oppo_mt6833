	...
	struct VL53LX_Dev_t *Dev)
{
	...
	struct VL53LX_ll_driver_state_t  *pstate       = &(pdev->ll_state);
	struct VL53LX_system_results_t   *psys_results = &(pdev->sys_results);
	struct VL53LX_histogram_bin_data_t *phist_data = &(pdev->hist_data);
	struct VL53LX_zone_private_dyn_cfgs_t *pZ = &(pres->zone_dyn_cfgs);
	...
	struct VL53LX_Dev_t *Dev)
{
	...
	struct VL53LX_zone_private_dyn_cfgs_t *pdata = &(pres->zone_dyn_cfgs);
	...
	struct VL53LX_Dev_t *Dev)
{
	...
	struct VL53LX_histogram_config_t *phist_cfg = &(pdev->hist_cfg);
...
struct VL53LX_GPIO_interrupt_config_t VL53LX_decode_GPIO_interrupt_config(
	uint8_t		system__interrupt_config)
{
	struct VL53LX_GPIO_interrupt_config_t	intconf;
	...
	)
{
	...
	struct VL53LX_LLDriverData_t *pdev = VL53LXDevStructGetLLDriverHandle(Dev);
	struct VL53LX_LLDriverResults_t *pres = VL53LXDevStructGetLLResultsHandle(Dev);
	struct VL53LX_smudge_corrector_config_t *pconfig =
				&(pdev->smudge_correct_config);
	struct VL53LX_smudge_corrector_internals_t *pint =
				&(pdev->smudge_corrector_internals);

	struct VL53LX_range_results_t *presults = &(pres->range_results);
	struct VL53LX_range_data_t *pxmonitor = &(presults->xmonitor);
	...
	)
{
	...
	struct VL53LX_xtalk_config_t  *pX = &(pdev->xtalk_cfg);
	struct VL53LX_xtalk_calibration_results_t  *pC = &(pdev->xtalk_cal);
	uint32_t *pcpo;
	...
	)
{
	...
	struct VL53LX_range_results_t  *pR = &(pres->range_results);
	struct VL53LX_xtalk_config_t  *pX = &(pdev->xtalk_cfg);
	...
	)
{
	...
	struct VL53LX_smudge_corrector_data_t *pdata;
	...
	struct VL53LX_Dev_t *Dev)
{
	...
	struct VL53LX_system_results_t *pS = &(pdev->sys_results);
