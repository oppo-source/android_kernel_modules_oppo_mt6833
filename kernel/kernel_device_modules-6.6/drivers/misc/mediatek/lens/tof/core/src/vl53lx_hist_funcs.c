...
VL53LX_Error VL53LX_hist_process_data(
	...
	struct VL53LX_histogram_bin_data_t        *pbins_input,
	...
	uint8_t                            *HistMergeNumber)
{
	...
	struct VL53LX_hist_gen3_algo_private_data_t  *palgo_gen3 =
			(struct VL53LX_hist_gen3_algo_private_data_t *) pArea1;
	struct VL53LX_hist_gen4_algo_filtered_data_t *pfiltered4 =
			(struct VL53LX_hist_gen4_algo_filtered_data_t *) pArea2;

	struct VL53LX_hist_gen3_dmax_private_data_t   dmax_algo_gen3;
	struct VL53LX_hist_gen3_dmax_private_data_t  *pdmax_algo_gen3 =
						&dmax_algo_gen3;

	struct VL53LX_histogram_bin_data_t             bins_averaged;
	struct VL53LX_histogram_bin_data_t           *pbins_averaged = &bins_averaged;
		...
		for (t = 0 ; t < presults->active_results ; t++) {
			...
			ppost_cfg->algo__crosstalk_detect_min_max_tolerance) {
				...
				memcpy(
					&(presults->xmonitor),
					pdata,
					sizeof(struct VL53LX_range_data_t));
	...
	int16_t                            *pambient_dmax_mm)
{
	...
	struct VL53LX_hist_gen3_dmax_private_data_t   dmax_algo;
	struct VL53LX_hist_gen3_dmax_private_data_t  *pdmax_algo = &dmax_algo;
