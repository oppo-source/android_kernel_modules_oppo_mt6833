	...
	struct VL53LX_xtalk_calibration_results_t	*pxtalk_cal)
{
	...
	struct VL53LX_xtalk_algo_data_t xtalk_debug;
	struct VL53LX_xtalk_algo_data_t *pdebug      = &xtalk_debug;
	struct VL53LX_xtalk_range_data_t *pxtalk_data = NULL;

	struct VL53LX_histogram_bin_data_t avg_bins;
	struct VL53LX_histogram_bin_data_t *pavg_bins   = &avg_bins;
	...
	)
{
	...
	struct VL53LX_xtalk_range_data_t  *presults_int = NULL;
	...
	)
{
	...
	memcpy(&(pdebug->VL53LX_p_057),
		phist_data, sizeof(struct VL53LX_histogram_bin_data_t));
