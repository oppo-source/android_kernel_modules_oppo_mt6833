...
static VL53LX_Error select_offset_per_vcsel(struct VL53LX_LLDriverData_t *pdev,
		int16_t *poffset)
{
...
static void vl53lx_diff_histo_stddev(struct VL53LX_LLDriverData_t *pdev,
	struct VL53LX_histogram_bin_data_t *pdata, uint8_t timing, uint8_t HighIndex,
...
static void vl53lx_histo_merge(struct VL53LX_Dev_t *Dev,
...
VL53LX_Error VL53LX_get_version(
	...
	struct VL53LX_ll_version_t *pdata)
{
	...
	memcpy(pdata, &(pdev->version), sizeof(struct VL53LX_ll_version_t));
	...
	uint8_t           read_p2p_data)
{
	...
	struct VL53LX_zone_objects_t    *pobjects;
	...
	struct VL53LX_Dev_t        *Dev)
{
	...
	VL53LX_Error status       = VL53LX_ERROR_NONE;
	struct VL53LX_LLDriverData_t *pdev = VL53LXDevStructGetLLDriverHandle(Dev);
	struct VL53LX_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
	struct VL53LX_customer_nvm_managed_t *pN = &(pdev->customer);
	struct VL53LX_additional_offset_cal_data_t *pCD = &(pdev->add_off_cal_data);

	struct VL53LX_decoded_nvm_fmt_range_data_t fmt_rrd;
	...
	struct VL53LX_calibration_data_t            *pcal_data)
{
	...
	VL53LX_Error  status = VL53LX_ERROR_NONE;
	struct VL53LX_LLDriverData_t *pdev = VL53LXDevStructGetLLDriverHandle(Dev);
	struct VL53LX_xtalk_config_t *pC = &(pdev->xtalk_cfg);
	struct VL53LX_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
	struct VL53LX_customer_nvm_managed_t *pN = &(pdev->customer);
	...
	if (status == VL53LX_ERROR_NONE) {
		...
		memcpy(
			&(pdev->customer),
			&(pcal_data->customer),
			sizeof(struct VL53LX_customer_nvm_managed_t));
		...
		memcpy(
			...
			sizeof(struct VL53LX_dmax_calibration_data_t));
	...
	struct VL53LX_calibration_data_t      *pcal_data)
{
	...
	struct VL53LX_LLDriverData_t *pdev = VL53LXDevStructGetLLDriverHandle(Dev);
	struct VL53LX_xtalk_config_t *pC = &(pdev->xtalk_cfg);
	struct VL53LX_customer_nvm_managed_t *pCN = &(pcal_data->customer);
	...
	struct VL53LX_zone_config_t      *pzone_cfg)
{
	...
	memcpy(pzone_cfg, &(pdev->zone_cfg), sizeof(struct VL53LX_zone_config_t));
	...
	uint32_t                     inter_measurement_period_ms)
{
	...
	struct VL53LX_static_config_t        *pstatic       = &(pdev->stat_cfg);
	struct VL53LX_histogram_config_t     *phistogram    = &(pdev->hist_cfg);
	struct VL53LX_general_config_t       *pgeneral      = &(pdev->gen_cfg);
	struct VL53LX_timing_config_t        *ptiming       = &(pdev->tim_cfg);
	struct VL53LX_dynamic_config_t       *pdynamic      = &(pdev->dyn_cfg);
	struct VL53LX_system_control_t       *psystem       = &(pdev->sys_ctrl);
	struct VL53LX_zone_config_t          *pzone_cfg     = &(pdev->zone_cfg);
	struct VL53LX_tuning_parm_storage_t  *ptuning_parms = &(pdev->tuning_parms);
	...
	VL53LX_DeviceConfigLevel       device_config_level)
{
	...
	struct VL53LX_static_nvm_managed_t   *pstatic_nvm   = &(pdev->stat_nvm);
	struct VL53LX_customer_nvm_managed_t *pcustomer_nvm = &(pdev->customer);
	struct VL53LX_static_config_t        *pstatic       = &(pdev->stat_cfg);
	struct VL53LX_general_config_t       *pgeneral      = &(pdev->gen_cfg);
	struct VL53LX_timing_config_t        *ptiming       = &(pdev->tim_cfg);
	struct VL53LX_dynamic_config_t       *pdynamic      = &(pdev->dyn_cfg);
	...
	struct VL53LX_ll_driver_state_t  *pstate   = &(pdev->ll_state);
	struct VL53LX_customer_nvm_managed_t *pN = &(pdev->customer);
	...
	VL53LX_DeviceResultsLevel      device_results_level)
{
	...
	struct VL53LX_system_results_t   *psystem_results = &(pdev->sys_results);
	struct VL53LX_core_results_t     *pcore_results   = &(pdev->core_results);
	struct VL53LX_debug_results_t    *pdebug_results  = &(pdev->dbg_results);
	...
	struct VL53LX_range_results_t       *prange_results)
{
	...
	struct VL53LX_ll_driver_state_t *pstate   =
			&(pdev->ll_state);
	struct VL53LX_zone_config_t     *pzone_cfg =
			&(pdev->zone_cfg);
	struct VL53LX_zone_hist_info_t  *phist_info =
			&(pres->zone_hists.VL53LX_p_003[0]);

	struct VL53LX_dmax_calibration_data_t   dmax_cal;
	struct VL53LX_dmax_calibration_data_t *pdmax_cal = &dmax_cal;
	struct VL53LX_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
	struct VL53LX_xtalk_config_t *pC = &(pdev->xtalk_cfg);
	struct VL53LX_low_power_auto_data_t *pL = &(pdev->low_power_auto_data);
	struct VL53LX_histogram_bin_data_t *pHD = &(pdev->hist_data);
	struct VL53LX_customer_nvm_managed_t *pN = &(pdev->customer);
	struct VL53LX_zone_histograms_t *pZH = &(pres->zone_hists);
	struct VL53LX_xtalk_calibration_results_t *pXCR = &(pdev->xtalk_cal);
	uint8_t tmp8;
	uint8_t zid;
	uint8_t i;
...
UPDATE_DYNAMIC_CONFIG:
	...
	memcpy(
		prange_results,
		presults,
		sizeof(struct VL53LX_range_results_t));
		...
		struct VL53LX_histogram_bin_data_t *pdata)
{
	...
	struct VL53LX_static_nvm_managed_t   *pstat_nvm = &(pdev->stat_nvm);
	struct VL53LX_static_config_t        *pstat_cfg = &(pdev->stat_cfg);
	struct VL53LX_general_config_t       *pgen_cfg  = &(pdev->gen_cfg);
	struct VL53LX_timing_config_t        *ptim_cfg  = &(pdev->tim_cfg);
	struct VL53LX_range_results_t        *presults  = &(pres->range_results);
	...
	struct VL53LX_tuning_parameters_t           *ptun_data)
{
	...
	struct VL53LX_LLDriverData_t *pdev = VL53LXDevStructGetLLDriverHandle(Dev);
	struct VL53LX_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
	struct VL53LX_xtalkextract_config_t *pXC = &(pdev->xtalk_extract_cfg);
