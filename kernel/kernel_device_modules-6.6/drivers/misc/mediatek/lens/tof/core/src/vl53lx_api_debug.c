	...
	struct VL53LX_calibration_data_t *pdata)
{
	...
	if (sizeof(struct VL53LX_calibration_data_t) > buf_size)
		return VL53LX_ERROR_COMMS_BUFFER_TOO_SMALL;

	memcpy(pdata, pbuffer, sizeof(struct VL53LX_calibration_data_t));
	...
	struct VL53LX_xtalk_debug_data_t          *pdata)
{
	...
	memcpy(
		&(pdata->xtalk_cfg),
		&(pdev->xtalk_cfg),
		sizeof(struct VL53LX_xtalk_config_t));
	...
	memcpy(
		&(pdata->xtalk_results),
		&(pdev->xtalk_results),
		sizeof(struct VL53LX_xtalk_range_results_t));
	...
	struct VL53LX_offset_debug_data_t         *pdata)
{
	...
	memcpy(
		...
		&(pdev->offset_results),
		sizeof(struct VL53LX_offset_range_results_t));
...
void VL53LX_print_offset_range_results(
	struct VL53LX_offset_range_results_t *pdata,
	char                          *pprefix,
	uint32_t                       trace_flags)
{
	...
	uint32_t                      trace_flags)
{
	...
	struct VL53LX_histogram_bin_data_t *pbin_data;
