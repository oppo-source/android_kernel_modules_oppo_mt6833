...
static struct lpm_spm_wake_status mt6877_wakesrc;

static struct lpm_log_helper mt6877_log_help = {
	.wakesrc = &mt6877_wakesrc,
	.cur = 0,
	.prev = 0,
};
#define IRQ_NUMBER	\
...
{
	struct lpm_log_helper *help = &mt6877_log_help;
...
{
	struct lpm_spm_wake_status *wakesrc = mt6877_log_help.wakesrc ;
...
#undef LOG_BUF_SIZE
	...
	if (log_type < LOG_SUCCEESS) {
		pr_info("[name:spm&][SPM] %s didn't enter MCUSYS off, cstate enter func ret = %d\n",
					prefix, log_type);
		wr =  WR_ABORT;
	...
	if (log_type == LOG_MCUSYS_NOT_OFF) {
		pr_info("[name:spm&][SPM] %s didn't enter MCUSYS off, MCUSYS cnt is no update\n",
					prefix);
		wr =  WR_ABORT;
...
static struct lpm_dbg_plat_ops mt6877_dbg_ops = {
...
int mt6877_dbg_ops_register(void)
{
	int ret;

	ret = lpm_dbg_plat_ops_register(&mt6877_dbg_ops);
