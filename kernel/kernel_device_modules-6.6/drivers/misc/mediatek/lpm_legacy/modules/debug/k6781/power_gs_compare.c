...
int mt6781_pwr_gs_set(unsigned int type, const struct lpm_data *val)
{
	int ret = 0;
...
struct lpm_callee mt6781_pwr_gs_callee = {
...
#ifdef MTK_LPM_GS_PLAT_CLK_DUMP_SUPPORT
struct lpm_gs_clk mt6781_clk_cg = {
	.type = GS_CG,
	.name = "CG",
};

struct lpm_gs_clk mt6781_clk_dcm = {
	.type = GS_DCM,
	.name = "DCM",
};

struct lpm_gs_clk *mt6781_clks[] = {
	&mt6781_clk_dcm,
	&mt6781_clk_cg,
	NULL,
};

int mt6781_power_gs_clk_user_attach(struct lpm_gs_clk *p)
{
...
struct lpm_gs_clk_info mt6781_clk_infos = {
...
/* PMIC */
struct lpm_gs_pmic mt6781_pmic6366 = {
	.type = GS_PMIC,
	.regulator = "vcore",
	.pwr_domain = "6366",
};

struct lpm_gs_pmic *mt6781_pmic[] = {
	&mt6781_pmic6366,
	NULL,
};

int mt6781_power_gs_pmic_user_attach(struct lpm_gs_pmic *p)
{
	if (!p || !p->regulator)
		return -EINVAL;
...
struct lpm_gs_pmic_info mt6781_pmic_infos = {
	.pmic = mt6781_pmic,
...
{
	lpm_callee_registry(&mt6781_pwr_gs_callee);

	/* initial gs compare method */
	lpm_pwr_gs_common_init();

	/* initial gs pmic information */
	lpm_pwr_gs_compare_init(LPM_GS_CMP_PMIC, &mt6781_pmic_infos);

#ifdef MTK_LPM_GS_PLAT_CLK_DUMP_SUPPORT
	/* initial gs dcm information */
	lpm_pwr_gs_compare_init(LPM_GS_CMP_CLK, &mt6781_clk_infos);
#endif
	return 0;
}
