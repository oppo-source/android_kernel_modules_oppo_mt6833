...
#include "mtk_cm_mgr_mt6781.h"
#include "mtk_cm_mgr_common.h"

/* #define CREATE_TRACE_POINTS */
/* #include "mtk_cm_mgr_events_mt6781.h" */
...
u32 cm_mgr_get_perfs_mt6781(int num)
{
	if (num < 0 || num >= cm_mgr_get_num_perf())
		return 0;
	return cm_mgr_perfs[num];
}
EXPORT_SYMBOL_GPL(cm_mgr_get_perfs_mt6781);
...
void cm_mgr_perf_platform_set_status_mt6781(int enable)
{
	...
	if (enable) {
		...
		if (cm_mgr_get_dram_opp_base() == -1) {
			...
			icc_set_bw(cm_mgr_get_bw_path(), 0,
					cm_mgr_get_perfs_mt6781(cm_mgr_dram_opp));
		} else {
...
EXPORT_SYMBOL_GPL(cm_mgr_perf_platform_set_status_mt6781);
...
void cm_mgr_perf_set_status_mt6781(int enable)
{
	...
	cm_mgr_perf_platform_set_status_mt6781(enable);
		}
EXPORT_SYMBOL_GPL(cm_mgr_perf_set_status_mt6781);

void cm_mgr_perf_set_force_status_mt6781(int enable)
{
...
EXPORT_SYMBOL_GPL(cm_mgr_perf_set_force_status_mt6781);

void check_cm_mgr_status_mt6781(unsigned int cluster, unsigned int freq,
		unsigned int idx)
{
	if (!cm_mgr_init_done)
		return;
...
EXPORT_SYMBOL_GPL(check_cm_mgr_status_mt6781);
...
static int platform_cm_mgr_probe(struct platform_device *pdev)
{
	...
	local_hk.cm_mgr_get_perfs =
		cm_mgr_get_perfs_mt6781;
	local_hk.cm_mgr_perf_set_force_status =
		cm_mgr_perf_set_force_status_mt6781;
	local_hk.check_cm_mgr_status =
		check_cm_mgr_status_mt6781;
	local_hk.cm_mgr_perf_platform_set_status =
		cm_mgr_perf_platform_set_status_mt6781;
	local_hk.cm_mgr_perf_set_status =
		cm_mgr_perf_set_status_mt6781;
...
static const struct of_device_id platform_cm_mgr_of_match[] = {
	{ .compatible = "mediatek,mt6781-cm_mgr", },
	{},
};
