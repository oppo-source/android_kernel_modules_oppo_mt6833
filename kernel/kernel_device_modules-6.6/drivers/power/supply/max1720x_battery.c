...
struct max1720x_priv {
	...
	struct task_struct *cs_thread_task;
	...
	spinlock_t slock;
	struct wakeup_source *cs_wakelock;
	bool cs_thread_timeout;
	bool cs_thread_polling;
	wait_queue_head_t  wait_que;
	struct alarm cs_timer;
	bool is_suspend;
...
static const u16 max17332_regs[] = {
	...
	[TTF_REG] = 0x20,
	[VERSION_REG] = 0x21,
	[CONSTANT_VOL_REG] = 0x2A,
	[CONSTANT_CUR_REG] = 0x28,
	[VEMPTY_REG] = 0x3A,
	[DIETEMP_REG] = 0x34,
	[AVGDIETEMP_REG] = 0x40,
	[QH_REG] = 0x4D,
...
static const u16 dump_init_setting[] = {
	0x180,
	0x181,
	0x182,
	0x183,
...
static enum power_supply_property max17332_cs_props[] = {
						  ...
						  struct device_attribute *attr, char *buf)
{
	...
	for (reg = 0x80; reg < priv->nvmem_high_addr; reg += 16) {
		...
		for (i = 0; i < 16; i++) {
			...
			if (ret < 0) {
				dev_info(dev, "NVM reading failed (%d)\n",
						ret);
				return ret;
			}
						 ...
						 struct device_attribute *attr, char *buf)
{
	...
	ret = max1720x_regmap_read(priv, reg, &val);
	if (ret < 0) {
		dev_info(dev, "cel 1 reading failed (%d)\n",
				ret);
		return ret;
	}
	voltage_val = (val * 625) / 8; /* CELL LSB: 78.125 uV */
	rc += snprintf(buf + rc, PAGE_SIZE - rc, "Cel_1 : %d\n", voltage_val);
	...
	if (priv->driver_data == ID_MAX1720X ||
		...
		for (i = 1; i < 4; i++) {
			...
			if (ret < 0) {
				dev_info(dev, "CEL %d reading failed (%d)\n",
						i + 1, ret);
				return ret;
			}
			voltage_val = (val * 625) / 8; /* CELL LSB: 78.125 uV */
			rc += snprintf(buf + rc, PAGE_SIZE - rc, "Cel_%d : %d\n", (i + 1), voltage_val);
		}
	}
	return rc;
}
						 ...
						 struct device_attribute *attr, char *buf)
{
	...
	if (priv->driver_data != ID_MAX1720X) {
		...
		ret = max1720x_regmap_read(priv, reg, &val);
		if (ret < 0) {
			dev_info(dev, "DT reading failed (%d)\n",
					ret);
			return ret;
		}
		temp_val = val / 256; /* TEMP LSB: 1/256 Celsius */
		rc += snprintf(buf + rc, PAGE_SIZE - rc, "DT : %d\n", temp_val);
	}

	reg = priv->regs[TEMP1_REG];
	ret = max1720x_regmap_read(priv, reg, &val);
	if (ret < 0) {
		dev_info(dev, "Temp 1 reading failed (%d)\n",
				ret);
		return ret;
	}
	temp_val = val / 256; /* TEMP LSB: 1/256 Celsius */
	...
	if (priv->driver_data == ID_MAX17320) {
		for (i = 1; i < 4; i++) {
			ret = max1720x_regmap_read(priv, reg - i, &val);
			if (ret < 0) {
				dev_info(dev, "Temp %d reading failed (%d)\n",
						i + 1, ret);
				return ret;
			}
			temp_val = val / 256; /* TEMP LSB: 1/256 Celsius */
...
/* mediatek implement */
static int max17332_get_cc(struct charger_device *chg_dev,
							u32 *cc)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	int ret = 0;

	ret = max1720x_regmap_read(priv, priv->regs[CONSTANT_CUR_REG], cc);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to get CC\n", __func__);
	...
	*cc = (int) (*cc / 100000);
	...
	dev_info(&(chg_dev->dev), "%s: cc=%d\n", __func__, *cc);

	return ret;
}

static int max17332_set_cc(struct charger_device *chg_dev,
								u32 cc)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	int ret = 0;
	...
	dev_info(&(chg_dev->dev), "%s: check\n", __func__);
	// Update ManChg mode
	ret = max1720x_regmap_read(priv, priv->regs[CONFIG_REG], &read_val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read manChg\n", __func__, ret);
	...
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to write manChg\n", __func__, ret);

	ret = max1720x_regmap_read(priv, priv->regs[CONFIG_REG], &read_val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read manChg\n", __func__, ret);
	dev_info(&(chg_dev->dev), "%s: read_manChg=%x\n", __func__, read_val);

	/* set cc */
	tmp2 = ((long)cc * 100000);
	cc = (u32)(tmp2 / 15625);
	ret = max1720x_regmap_write(priv, priv->regs[CONSTANT_CUR_REG], cc);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to write CC\n", __func__, ret);
	dev_info(&(chg_dev->dev), "%s: %x, %d\n", __func__, cc, cc);

	ret = max1720x_regmap_read(priv, priv->regs[CONSTANT_CUR_REG], &tmp);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read CC\n", __func__, ret);
	dev_info(&(chg_dev->dev), "%s: read_cc=%x\n", __func__, tmp);

	return ret;
}

static int max17332_get_vfocv(struct charger_device *chg_dev,
								u32 *vfocv)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	int ret = 0;

	ret = max1720x_regmap_read(priv, priv->regs[VFOCV_REG], vfocv);
	dev_info(&(chg_dev->dev), "%s: vfocv=%x\n", __func__, *vfocv);
	*vfocv = (*vfocv * 625 ) / 8;
	*vfocv /= 1000;
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to get VFOCV\n", __func__);
	return (int) *vfocv;
}

static int max17332_get_cv(struct charger_device *chg_dev,
								u32 *cv)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	int ret = 0;

	ret = max1720x_regmap_read(priv, priv->regs[CONSTANT_VOL_REG], cv);
	dev_info(&(chg_dev->dev), "%s: cv=%x\n", __func__, *cv);
	*cv = (*cv * 625 ) / 8;
	*cv /= 1000;
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to get CV\n", __func__, ret);
	return ret;
}

static int max17332_set_cv(struct charger_device *chg_dev,
								u32 cv)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	int ret = 0;
	...
	ret = max1720x_regmap_read(priv, priv->regs[CONSTANT_VOL_REG], &tmp);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read CV\n", __func__);
	dev_info(&(chg_dev->dev), "%s: read_cv=%x\n", __func__, tmp);
	...
	// set cv
	cv = (cv * 8000) / 625; // 0.078125 mV
	//cv = (int)(tmp * 1000);
	ret = max1720x_regmap_write(priv, priv->regs[CONSTANT_VOL_REG], cv);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to write CV\n", __func__);
...
static int max17332_get_vbus(struct charger_device *chg_dev,
								u32 *vbus)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	...
	val = (val * 625) / 2; /* BATT LSB: 312.5 uV */
	*vbus = (u32) val / 1000;
	return ret;
}

static int max17332_get_vbat(struct charger_device *chg_dev,
								u32 *vbat)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	...
	dev_info(&(chg_dev->dev), "%s: 0x%04x\n", __func__, val);
	val = (val * 625) / 2; // BATT LSB: 312.5 uV
	*vbat = (u32) val / 1000;
	return ret;
}

static int max17332_get_ibat(struct charger_device *chg_dev,
								int *ibat)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	...
	ret = max1720x_regmap_read(priv, priv->regs[CURRENT_REG], &val);
	dev_info(&(chg_dev->dev), "%s: %x\n", __func__, val);
	val = val & 0xFFFF;
	...
	tmp = ((long) val) * 15625;
	*ibat = (int) (tmp / 100000);
	dev_info(&(chg_dev->dev), "%s: %d, %d\n", __func__, *ibat, val);
	return ret;
}

static int max17332_get_ibat_avg(struct charger_device *chg_dev,
								int *ibat_avg)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	...
	ret = max1720x_regmap_read(priv, priv->regs[AVGCURRENT_REG], &val);
	val = val & 0xFFFF;
	if ((val & 0x8000) == 0x8000) {
		val = ((~val) + 1) & 0xFFFF;
		val = val * (-1);
	...
	tmp = ((long) val) * 15625;
	*ibat_avg = (int) (tmp / 100000);
	dev_info(&(chg_dev->dev), "%s: %d, %d\n", __func__, *ibat_avg, val);
	return ret;
}

static int max17332_get_ichg_term(struct charger_device *chg_dev,
								int *ichg_term)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	...
	ret = max1720x_regmap_read(priv, priv->regs[NICHGTERM_REG], &val);
	val = val & 0xFFFF;
	if ((val & 0x8000) == 0x8000) {
		val = ((~val) + 1) & 0xFFFF;
		val = val * (-1);
	...
	tmp = ((long) val) * 15625;
	*ichg_term = (int) (tmp / 100000);
	dev_info(&(chg_dev->dev), "%s: %d, %d\n", __func__, *ichg_term, val);
	return ret;
}

static int max17332_get_vcell(struct charger_device *chg_dev,
								u32 *vcell)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
...
static int max17332_get_vcell_avg(struct charger_device *chg_dev,
								u32 *vcell_avg)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	...
	val = (val * 625) / 2; // BATT LSB: 312.5 uV
	*vcell_avg = (u32) val / 1000;
	return ret;
}

static int max17332_get_temp(struct charger_device *chg_dev,
								int *temp)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	...
	ret = max1720x_regmap_read(priv, priv->regs[TEMP_REG], &val);
	val = val >> 8;
	*temp = val;
	return ret;
}

static int max17332_get_temp_avg(struct charger_device *chg_dev,
								int *temp_avg)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	...
	ret = max1720x_regmap_read(priv, priv->regs[AVGTEMP_REG], &val);
	val = val >> 8;
	*temp_avg = val;
	return ret;
}

static int max17332_get_dietemp(struct charger_device *chg_dev,
								int *dietemp)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	...
	ret = max1720x_regmap_read(priv, priv->regs[DIETEMP_REG], &val);
	val = val >> 8;
	*dietemp = val;
	return ret;
}

static int max17332_get_dietemp_avg(struct charger_device *chg_dev,
								int *dietemp_avg)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	...
	ret = max1720x_regmap_read(priv, priv->regs[AVGDIETEMP_REG], &val);
	val = val >> 8;
	*dietemp_avg = val;
	return ret;
}

static int max17332_get_fettemp(struct charger_device *chg_dev,
								int *fettmp)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	...
	ret = max1720x_regmap_read(priv, priv->regs[FET_TEMP], &val);
	val = val >> 8;
	*fettmp = val;
	return ret;
}

static int max17332_dump_init_setting(struct charger_device *chg_dev)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	int i = 0;
	int ret = 0;
	...
	for (i = 0; i < 111; i++) {
		ret = max1720x_regmap_read(priv, dump_init_setting[i], &val);
		if (ret < 0)
			dev_info(&(chg_dev->dev), "%s failed to get log:%d\n", __func__, i);
		dev_info(&(chg_dev->dev), "0x%04X = 0x%04X\n", dump_init_setting[i], val);
	}
	return ret;
}

static int max17332_init_setting(struct charger_device *chg_dev)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;
	int FULLTIMER_POS = 11;
	int FULLTIMER_SET = 7;
	int BATEXIST_MASK = 1 << 3;
	int ret = 0;
	int val = 0;
	...
	if (!priv) {
		dev_info(&(chg_dev->dev), "max17332 init failed\n");
		pdata->disable_cs = 1;
		return -1;
	}

	ret = max1720x_regmap_read(priv, priv->regs[STATUS_REG], &val);
	if (ret < 0) {
		dev_info(&(chg_dev->dev), "%s failed to read NDELAYCFG_REG.\n", __func__);
		pdata->disable_cs = 1;
		return -1;
	}
	if (val & BATEXIST_MASK) {
		dev_info(&(chg_dev->dev), "no Battery on cs side!\n");
		pdata->disable_cs = 1;
		return -1;
	}
	...
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read NDELAYCFG_REG.\n", __func__);
	dev_info(&(chg_dev->dev), "%s NDELAYCFG_REG: 0x%04X.\n", __func__, val);
	val = val & ~(FULLTIMER_SET << FULLTIMER_POS);
	ret = max1720x_regmap_write(priv, priv->regs[NDELAYCFG_REG], val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to write NDELAYCFG_REG.\n", __func__);
	...
	/* for recharge */
	val = 0x000D;
	ret = max1720x_regmap_write(priv, priv->regs[NFULLCFG_REG], val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to write NFULLCFG_REG.\n", __func__);

	ret = max1720x_regmap_read(priv, priv->regs[NFULLCFG_REG], &val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read NFULLCFG_REG.\n", __func__);
	dev_info(&(chg_dev->dev), "%s NFULLCFG_REG: 0x%04X.\n", __func__, val);

	/* for IEOC */
	val = 0x03C0;
	ret = max1720x_regmap_write(priv, priv->regs[NICHGTERM_REG], val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to write NICHGTERM_REG.\n", __func__);

	ret = max1720x_regmap_read(priv, priv->regs[NICHGTERM_REG], &val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read NICHGTERM_REG.\n", __func__);
	dev_info(&(chg_dev->dev), "%s NICHGTERM_REG: 0x%04X.\n", __func__, val);

	/* for full alert */
	val = 0x3010;
	ret = max1720x_regmap_write(priv, priv->regs[NPROTCFG_REG], val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to write NPROTCFG_REG.\n", __func__);

	ret = max1720x_regmap_read(priv, priv->regs[NPROTCFG_REG], &val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read NPROTCFG_REG.\n", __func__);
	dev_info(&(chg_dev->dev), "%s NPROTCFG_REG: 0x%04X.\n", __func__, val);

	/* for Current Detection */
	val = 0x7A2F;
	ret = max1720x_regmap_write(priv, priv->regs[NPROTMISCTH_REG], val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to write NPROTMISCTH_REG.\n", __func__);

	ret = max1720x_regmap_read(priv, priv->regs[NPROTMISCTH_REG], &val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read NPROTMISCTH_REG.\n", __func__);
	dev_info(&(chg_dev->dev), "%s NPROTMISCTH_REG: 0x%04X.\n", __func__, val);

	/* for Thermistor setting */
	val = 0x1000;
	ret = max1720x_regmap_write(priv, priv->regs[NPACKCFG_REG], val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to write NPACKCFG_REG.\n", __func__);

	ret = max1720x_regmap_read(priv, priv->regs[NPACKCFG_REG], &val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read NPACKCFG_REG.\n", __func__);
	dev_info(&(chg_dev->dev), "%s NPACKCFG_REG: 0x%04X.\n", __func__, val);

		/* for nVChgCfg1 setting */
	val = 0x0180;
	ret = max1720x_regmap_write(priv, priv->regs[NVCHGCFG1_REG], val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to write NVCHGCFG1_REG.\n", __func__);

	ret = max1720x_regmap_read(priv, priv->regs[NVCHGCFG1_REG], &val);
	if (ret < 0)
		dev_info(&(chg_dev->dev), "%s failed to read NVCHGCFG1_REG.\n", __func__);
	dev_info(&(chg_dev->dev), "%s NVCHGCFG1_REG: 0x%04X.\n", __func__, val);

	return ret;
}

void _wake_up_cs_allowchgB(struct max1720x_priv *info)
{
	unsigned long flags;
	...
	if (!info->cs_wakelock->active)
		__pm_stay_awake(info->cs_wakelock);
	spin_unlock_irqrestore(&info->slock, flags);
	info->cs_thread_timeout = true;
	wake_up_interruptible(&info->wait_que);
}

static enum alarmtimer_restart cs_alarm_timer_func(struct alarm *alarm, ktime_t now)
{
	struct max1720x_priv *info = container_of(alarm, struct max1720x_priv, cs_timer);

	if (info->is_suspend == false) {
		dev_info(&(info->chg_dev->dev), "%s: not suspend, wake up charger\n", __func__);
		_wake_up_cs_allowchgB(info);
	} else {
		dev_info(&(info->chg_dev->dev), "%s: alarm timer timeout\n", __func__);
		__pm_stay_awake(info->cs_wakelock);
	}
...
static void cs_start_timer(struct max1720x_priv *info)
{
	struct timespec64 end_time, time_now;
	ktime_t ktime, ktime_now;
	int ret = 0;

	ret = alarm_try_to_cancel(&info->cs_timer);
	if (ret < 0) {
		dev_info(&(info->chg_dev->dev), "%s: callback was running, skip timer\n", __func__);
		return;
	}
	...
	dev_info(&(info->chg_dev->dev), "%s: alarm timer start:%d, %ld %ld\n", __func__, ret,
		info->endtime.tv_sec, info->endtime.tv_nsec);
	alarm_start(&info->cs_timer, ktime);
}

static void cs_init_timer(struct max1720x_priv *info)
{
	alarm_init(&info->cs_timer, ALARM_BOOTTIME,
			cs_alarm_timer_func);
	cs_start_timer(info);

#ifdef CONFIG_PM
	if (register_pm_notifier(&info->pm_notifier))
		dev_info(&(info->chg_dev->dev), "%s: register pm failed\n", __func__);
#endif
}

static int cs_allowchgB_threadfn(void *param)
{
	struct max1720x_priv *priv = param;
	int ALLOWCHGB_POS = 5;
	int ALLOWCHGB_EN = 1 << ALLOWCHGB_POS;
	int ret = 0;
	int val = 0;

	while(1) {
		ret = wait_event_interruptible(priv->wait_que,
				(priv->cs_thread_timeout == true));
		if (priv->cs_thread_polling == true)
			cs_start_timer(priv);

		ret = max1720x_regmap_read(priv, priv->regs[STATUS_REG], &val);
		if (ret < 0)
			dev_info(&(priv->chg_dev->dev), "%s failed to read STATUS_REG.\n", __func__);
		dev_info(&(priv->chg_dev->dev), "%s STATUS_REG: 0x%04X.\n", __func__, val);
		if (val & (ALLOWCHGB_EN))
			val = val & ~(ALLOWCHGB_EN);
		ret = max1720x_regmap_write(priv, priv->regs[STATUS_REG], val);
		if (ret < 0)
			dev_info(&(priv->chg_dev->dev), "%s failed to write STATUS_REG.\n", __func__);
		priv->cs_thread_timeout = false;
	}
	return 0;
}

static int max17332_dump_registers(struct charger_device *chg_dev)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < 27; i++) {
		...
		index = dump_reg[i];
		switch (index) {
		case CONSTANT_VOL_REG:
			ret = max17332_get_cv(chg_dev, &val);
			break;
		case CONSTANT_CUR_REG:
			ret = max17332_get_cc(chg_dev, &val);
			break;
		case VCELL_REG:
			ret = max17332_get_vcell(chg_dev, &val);
			break;
		case AVGVCELL_REG:
			ret = max17332_get_vcell_avg(chg_dev, &val);
			break;
		case PCKP_REG:
			ret = max17332_get_vbus(chg_dev, &val);
			break;
		case VFOCV_REG:
			ret = max17332_get_vfocv(chg_dev, &val);
			break;
		case CURRENT_REG:
			ret = max17332_get_ibat(chg_dev, &val);
			break;
		case AVGCURRENT_REG:
			ret = max17332_get_ibat_avg(chg_dev, &val);
			break;
		case ICHG_TERM:
			ret = max17332_get_ichg_term(chg_dev, &val);
			break;
		case TEMP_REG:
			ret = max17332_get_temp(chg_dev, &val);
			break;
		case AVGTEMP_REG:
			ret = max17332_get_temp_avg(chg_dev, &val);
			break;
		case DIETEMP_REG:
			ret = max17332_get_dietemp(chg_dev, &val);
			break;
		case AVGDIETEMP_REG:
			ret = max17332_get_dietemp_avg(chg_dev, &val);
			break;
		case FET_TEMP:
			ret = max17332_get_fettemp(chg_dev, &val);
			break;
		default:
			ret = max1720x_regmap_read(priv, priv->regs[index], &val);
			break;
		}
		if (ret < 0)
			dev_info(&(chg_dev->dev), "%s failed to get log:%d\n", __func__, i);
	...
	dev_info(&(chg_dev->dev), "max17332_dump_reg: STATUS: 0x%04X, PROTALRTS: 0x%04X, PROTSTATUS: 0x%04X, HPROT_CFG: 0x%04X, HPROT_CFG2: 0x%04X, CHG_STATUS: 0x%04X, CONFIG: 0x%04X, CONFIG2: 0x%04X, CV: %d, CC: %d, NFULLCFG: 0x%04X, BLOCK_DIS: 0x%04X\n",
			val_list[0], val_list[1], val_list[2], val_list[3], val_list[4], val_list[5], val_list[6],
			val_list[7], val_list[8], val_list[9], val_list[10], val_list[11]);
	dev_info(&(chg_dev->dev), "max17332_dump_reg: VCELL: %d, AVG_VCELL: %d, PCKP: %d, VFOCV: %d, CURRENT: %d, AVG_CURRENT:%d, ICHG_TERM:%d, TEMP: %d, AVG_TEMP: %d, DIE_TEMP: %d, AVG_DIE_TEMP: %d, FET_TEMP: %d, NPACK: %04X, NDELAYCFG: %04X, COMMSTAT: %04X\n",
			val_list[12], val_list[13], val_list[14], val_list[15], val_list[16],
			val_list[17], val_list[18], val_list[19], val_list[20], val_list[21],
			val_list[22], val_list[23], val_list[24], val_list[25], val_list[26]);
	return ret;
}
									  ...
									  struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_info(dev, "Protection Status Register reading failed (%d)\n", ret);
		return ret;
	}
	...
	if (val) {
		...
		if (val & MAX17320_PROTSTATUS_DIEHOT)
			rc += snprintf(buf + rc, PAGE_SIZE - rc,
			"Protection Alert: Overtemperature for die temperature!\n");
		if (val & MAX17320_PROTSTATUS_TOOHOTD)
			rc += snprintf(buf + rc, PAGE_SIZE - rc,
			"Protection Alert: Overtemperature for Discharging!\n");
									  ...
									  struct device_attribute *attr, char *buf)
{
	...
	if (val) {
		...
		if (val & MAX17330_PROTSTATUS_UVP)
			rc += snprintf(buf + rc, PAGE_SIZE - rc,
			"Protection Alert: Undervoltage Protection!\n");
		if (val & MAX17330_PROTSTATUS_ODCP)
			rc += snprintf(buf + rc, PAGE_SIZE - rc,
			"Protection Alert: Overdischarge current!\n");
	} else {
		...
		if (ret < 0) {
			dev_info(dev, "Protection Status Register clearing failed (%d)\n", ret);
			return ret;
		}
						...
						struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_info(dev, "CommStat Register reading failed (%d)\n", ret);
		return ret;
	}
						 ...
						 struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_info(dev, "Config Register reading failed (%d)\n", ret);
		return ret;
	}
							 ...
							 struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_info(dev, "Register reading failed (%d)\n", ret);
		rc = ret;
	} else {
								 ...
								 struct device_attribute *attr, char *buf)
{
	...
	ret = max1720x_regmap_read(priv, priv->regs[NDELAYCFG_REG], &val);
	if (ret < 0) {
		dev_info(dev, "Delay Config Register reading failed (%d)\n", ret);
		return ret;
	}
							...
							const char *buf, size_t count)
{
	...
	if (priv->driver_data != ID_MAX17332) {
		...
		if (ret < 0) {
			dev_info(dev, "RSense Register reading failed (%d)\n", ret);
			return ret;
		}
									 ...
									 struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_info(dev, "nVEmpty Register reading failed (%d)\n", ret);
		return ret;
	}
						 ...
						 struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_info(dev, "nOVPrtTh Register reading failed (%d)\n", ret);
		return ret;
	}
						  ...
						  struct device_attribute *attr, char *buf)
{
	...
	rc += snprintf(buf + rc, PAGE_SIZE - rc,
	"Delta from Overvoltage Protection to the Overvoltage-Release Threshold : %d mV!\n", val);
								   ...
								   const char *buf, size_t count)
{
	...
	if (priv->driver_data == ID_MAX17330) {
		...
		read_val |= ((val - MAX17330_NOVPRTH_OVPPERMFAIL_MIN) / MAX17330_NOVPRTH_OVPPERMFAIL_STEP)
		<< MAX17330_NOVPRTTH_OVPPERMFAIL_POS;
	} else {
		read_val &= ~(MAX17332_NOVPRTTH_OVPPERMFAIL);
		read_val |= ((val - MAX17332_NOVPRTH_OVPPERMFAIL_MIN) / MAX17332_NOVPRTH_OVPPERMFAIL_STEP)
		<< MAX17332_NOVPRTTH_OVPPERMFAIL_POS;
	}
							  ...
							  const char *buf, size_t count)
{
	...
	if (priv->driver_data == ID_MAX17330) {
		...
		read_val |= ((val - MAX17330_NOVPRTH_CHGDETTH_MIN) / MAX17330_NOVPRTH_CHGDETTH_STEP)
		<< MAX17330_NOVPRTTH_CHGDETTH_POS;
		...
		read_val |= ((val - MAX17332_NIPRTTH2_CHGDETTH_MIN) / MAX17332_NIPRTTH2_CHGDETTH_STEP)
		<< MAX17332_NIPRTTH2_CHGDETTH_POS;
							 ...
							 struct device_attribute *attr, char *buf)
{
	...
	if (priv->driver_data == ID_MAX17330) {
		...
		if (ret < 0) {
			dev_info(dev, "nIPrtTh2 Register reading failed (%d)\n", ret);
			return ret;
		}
						 ...
						 struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_info(dev, "nODSCTh Register reading failed (%d)\n", ret);
		return ret;
	}
						 ...
						 struct device_attribute *attr, char *buf)
{
	...
	ret = max1720x_regmap_read(priv, priv->regs[NPACKCFG_REG], &val);
	if (ret < 0) {
		dev_info(dev, "nPackCfg Register reading failed (%d)\n", ret);
		return ret;
	}
						...
						struct device_attribute *attr, char *buf)
{
	...
	ret = max1720x_regmap_read(priv, priv->regs[NUVPRTTH_REG], &val);
	if (ret < 0) {
		dev_info(dev, "nUVPrtTh Register reading failed (%d)\n", ret);
		return ret;
	}
						  ...
						  struct device_attribute *attr, char *buf)
{
	...
	ret = max1720x_regmap_read(priv, priv->regs[NODSCCFG_REG], &val);
	if (ret < 0) {
		dev_info(dev, "nODSCCfg Register reading failed (%d)\n", ret);
		return ret;
	}
						   ...
						   const char *buf, size_t count)
{
	...
	if (priv->driver_data == ID_MAX17330) {
		...
		ret = max17330_threshold_temperature_store(priv,
		priv->regs[NTPRTTH3_REG], in_val, MAX17330_NTPRTTH3_TWARM_POS);
		if (ret < 0)
			return ret;
	} else {
		ret = max17332_threshold_temperature_store(priv,
		priv->regs[NTPRTTH2_REG], in_val, MAX17332_NTPRTTH2_TWARM_POS);
		if (ret < 0)
			return ret;
	}
	return count;
						  ...
						  struct device_attribute *attr, char *buf)
{
	...
	if (priv->driver_data == ID_MAX17330)
		ret = max17330_threshold_temperature_show(buf,
		priv, priv->regs[NTPRTTH3_REG], MAX17330_NTPRTTH3_TWARM_POS);
	else
		ret = max17332_threshold_temperature_show(buf,
		priv, priv->regs[NTPRTTH2_REG], MAX17332_NTPRTTH2_TWARM_POS);
								  ...
								  const char *buf, size_t count)
{
	...
	if (priv->driver_data == ID_MAX17330) {
		...
		ret = max17330_threshold_temperature_store(priv,
		priv->regs[NTPRTTH3_REG], in_val, MAX17330_NTPRTTH3_TPERMFAILHOT_POS);
		if (ret < 0)
			return ret;
	} else {
								 ...
								 struct device_attribute *attr, char *buf)
{
	...
	if (priv->driver_data == ID_MAX17330) {
		ret = max17330_threshold_temperature_show(buf,
		priv, priv->regs[NTPRTTH3_REG], MAX17330_NTPRTTH3_TPERMFAILHOT_POS);
	} else {
						...
						const char *buf, size_t count)
{
	...
	ret = max17332_threshold_temperature_store(priv,
	priv->regs[NTPRTTH1_REG], in_val, MAX17332_NTPRTTH1_TCOLD1_POS);
	if (ret < 0)
		return ret;
						...
						const char *buf, size_t count)
{
	...
	ret = max17332_threshold_temperature_store(priv,
	priv->regs[NTPRTTH1_REG], in_val, MAX17332_NTPRTTH1_TCOLD2_POS);
	if (ret < 0)
		return ret;
						...
						const char *buf, size_t count)
{
	...
	ret = max17332_threshold_temperature_store(priv,
	priv->regs[NTPRTTH2_REG], in_val, MAX17332_NTPRTTH2_TTOOHOT_POS);
	if (ret < 0)
		return ret;
								...
								struct device_attribute *attr, char *buf)
{
	...
	ret = max1720x_regmap_read(priv, priv->regs[NCHGCFG0_REG], &val);
	if (ret < 0) {
		dev_info(dev, "nChgCfg0 Register reading failed (%d)\n", ret);
		return ret;
	}
							...
							struct device_attribute *attr, char *buf)
{
	...
	ret = max1720x_regmap_read(priv, priv->regs[NCHGCFG1_REG], &val);
	if (ret < 0) {
		dev_info(dev, "nChgCfg1 Register reading failed (%d)\n", ret);
		return ret;
	}
								...
								struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_info(dev, "nConfig Register reading failed (%d)\n", ret);
		return ret;
	}
						...
						struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_info(dev, "nVCFG1 Register reading failed (%d)\n", ret);
		return ret;
	}
						...
						struct device_attribute *attr, char *buf)
{
	...
	rc += snprintf(buf + rc, PAGE_SIZE - rc, "Number of Updates Remaining : %d\n",
			(8 - number_of_used));
	return rc;
}
						...
						const char *buf, size_t len)
{
	...
	if (ret < 0 || !fw->data || !fw->size) {
		dev_info(dev, "Failed to get ini file\n");
		return ret;
	}
	...
	if (size != ((priv->nvmem_high_addr - base_addr + 1) * 4)) {
		dev_info(dev, "Firmware size is not correct!\n");
		release_firmware(fw);
		return ret;
	}
...
nvm_block_copy:
	...
	ret = regmap_read(priv->regmap, priv->regs[COMMSTAT_REG], &data);
	if (ret < 0) {
		dev_info(priv->dev, "Failed to read register commstat\n");
		goto error;
	}
	...
	if ((data & MAX17330_COMMSTAT_NVERROR) == MAX17330_COMMSTAT_NVERROR) {
		dev_info(priv->dev, "Failed to clear CommStat.NVError\n");
		goto error;
	}
	...
	for (i = 0; i < size; i++) {
		...
		if (data != memory_data) {
			...
			dev_info(priv->dev, "Failed to verfiy register 0x%X\n", address_data);
			goto error;
		}
	}
	priv->client->addr = MODELGAUGE_DATA_I2C_ADDR;
...
static DEVICE_ATTR_RW(attte);
static DEVICE_ATTR_RW(cell);
static DEVICE_ATTR_RW(temp);
static DEVICE_ATTR_RW(batt);
static DEVICE_ATTR_RW(leakcurr);
static DEVICE_ATTR_RW(max17320_protstat);
static DEVICE_ATTR_RW(max17330_protstat);
static DEVICE_ATTR_RW(register_store);
...
static DEVICE_ATTR_RW(ChgDetTh);
...
static DEVICE_ATTR_RW(TpermFaiilHot_show);
...
static DEVICE_ATTR_RW(PreQualVoltalVolt_store);
static DEVICE_ATTR_RW(VSysMin);
static DEVICE_ATTR_RW(PreChgCurr,urr_store);
static DEVICE_ATTR_RW(HeatLim);
static DEVICE_ATTR_RW(FETLim);
static DEVICE_ATTR_RW(FetTheta);
static DEVICE_ATTR_RW(push_button);
static DEVICE_ATTR_RW(enJ);
static DEVICE_ATTR_RW(control);
static DEVICE_ATTR_RW(remaining_nmv_updates);
static DEVICE_ATTR_RW(manChg);
static DEVICE_ATTR_RW(program_nvm_memory);
...
static int max17332_do_event(struct charger_device *chg_dev, u32 event, u32 args)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
...
static int max17332_cs_status_control(struct charger_device *chg_dev, bool enable)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	struct max1720x_platform_data *pdata = priv->pdata;

	if (!enable)
		gpio_set_value(pdata->gpios, 1);
	else
		gpio_set_value(pdata->gpios, 0);

	return 0;
}

static int max17332_chg_is_charge_done(struct charger_device *chg_dev, bool *done)
{
	int ret;
	...
	if (pdata->disable_cs)
		return -1;
	ret = power_supply_get_property(priv->psy, POWER_SUPPLY_PROP_STATUS,
					&val);
	if (ret < 0)
		return ret;
...
static int max17332_status_update(struct max1720x_priv *priv)
{
	int ret = 0;
	int CHGFET_POS = 1;
	int DISFET_POS = 1 << 1;
	int ISDIS_POS = 1 << 5;
	int FULLCHG_POS = 1 << 13;
	int tmp = 0;
	int tmp2 = 0;
	int val = 0;
	...
	ret = max1720x_regmap_read(priv, priv->regs[PROTSTATUS_REG], &val);
	if (val & FULLCHG_POS) {
		dev_info(priv->dev, "Battery full on max17332\n");
		return CS_STAT_FULL;
	}

	/* CHARGING */
	ret = max1720x_regmap_read(priv, priv->regs[HPROT_CFG2_REG], &val);
	tmp = val & CHGFET_POS;
	ret = max1720x_regmap_read(priv, priv->regs[FPROTSTAT_REG], &val);
	tmp2 = val & ISDIS_POS;
	if (tmp && !tmp2)
		return CS_STAT_CHARGING;
	...
	tmp2 = val & ISDIS_POS;
	if (tmp && tmp2)
		return CS_STAT_DISCHARGING;

	return CS_STAT_IDLE;
}

static int max17332_get_cs_status(struct max1720x_priv *priv)
{
	int stat = 0;

	stat = max17332_status_update(priv);

	switch (stat) {
	case CS_STAT_IDLE:
		return POWER_SUPPLY_STATUS_NOT_CHARGING;
	case CS_STAT_CHARGING:
		return POWER_SUPPLY_STATUS_CHARGING;
	case CS_STAT_FULL:
		return POWER_SUPPLY_STATUS_FULL;
	case CS_STAT_DISCHARGING:
...
static int max17332_check_cs_temp(struct charger_device *chg_dev)
{
	struct max1720x_priv *priv = dev_get_drvdata(&(chg_dev->dev));
	int ret = 0;
	int heatlim_val = 0;
	...
	ret = max17332_get_fettemp(chg_dev, &tmp_val);
	if (ret < 0) {
		dev_info(priv->dev, "Failed to read fettemp\n");
		return ret;
	}
	ret = max1720x_regmap_read(priv, priv->regs[NCHGCFG1_REG], &heatlim_val);
	if (ret < 0) {
		dev_info(priv->dev, "Failed to read HEATLIM\n");
		return ret;
	}

	dev_info(priv->dev, "FET_TMP: %d, HEATLIM: 0x%04x\n", tmp_val, heatlim_val);
	if (tmp_val >= 55) {
		dev_info(priv->dev, "FET_TMP over 55, close CS.\n", tmp_val);
		ret = max1720x_regmap_read(priv, priv->regs[NPROTCFG_REG], &val);
		if (ret < 0) {
			dev_info(priv->dev, "Failed to read NPROTCFG_REG\n");
			return ret;
		}

		max1720x_regmap_write(priv, priv->regs[NCHGCFG1_REG], 0x5000);
			if (ret < 0) {
				dev_info(priv->dev, "Failed to write HEATLIM\n");
				return ret;
			}
	...
	else if (tmp_val < 50 && priv->cs_heatlim == 2) {
		dev_info(priv->dev, "FET_TMP back to 50, open cs\n", tmp_val);
		...
		//max17332_par_setting(chg_dev, 0);
	...
	else if (tmp_val >= 47 && priv->cs_heatlim < 1) {
		dev_info(priv->dev, "FET_TMP over 47, throttling\n", tmp_val);
		max1720x_regmap_write(priv, priv->regs[NCHGCFG1_REG], 0x5000);
			if (ret < 0) {
				dev_info(priv->dev, "Failed to write HEATLIM\n");
				return ret;
		...
		priv->cs_heatlim = 1;
	} else if (tmp_val < 42 && priv->cs_heatlim > 0) {
		dev_info(priv->dev, "FET_TMP back to 42\n", tmp_val);
		max1720x_regmap_write(priv, priv->regs[NCHGCFG1_REG], 0xFE00);
		if (ret < 0) {
			dev_info(priv->dev, "Failed to write HEATLIM\n");
			return ret;
		}
	...
	dev_info(priv->dev, "cs_heat_level: %d\n", priv->cs_heatlim);
	return ret;
}

static int max17332_get_property(struct power_supply *psy,
								 ...
								 union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		ret = max17332_get_cs_status(priv);
		val->intval = ret;
		break;
...
static int max17332_set_property(struct power_supply *psy,
						enum power_supply_property psp,
						const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP_ALERT_MIN:
		...
		if (ret < 0)
			dev_info(priv->dev, "temp alert min set fail:%d\n",
					ret);
		break;
	case POWER_SUPPLY_PROP_TEMP_ALERT_MAX:
		ret = max1720x_set_temp_upper_limit(priv, val->intval);
		if (ret < 0)
			dev_info(priv->dev, "temp alert max set fail:%d\n",
					ret);
		break;
	case POWER_SUPPLY_PROP_CAPACITY_ALERT_MIN:
		ret = max1720x_set_min_capacity_alert_th(priv, val->intval);
		if (ret < 0)
			dev_info(priv->dev, "capacity alert min set fail:%d\n",
					ret);
		break;
	case POWER_SUPPLY_PROP_CAPACITY_ALERT_MAX:
		ret = max1720x_set_max_capacity_alert_th(priv, val->intval);
		if (ret < 0)
			dev_info(priv->dev, "capacity alert max set fail:%d\n",
					ret);
		break;
	default:
		return -EINVAL;
...
static int max1720x_init(struct max1720x_priv *priv)
{
	...
	u16 max17332_nvm_defaults[][2] = {
		{priv->regs[NRSENSE_REG], 0x000A},
		{priv->regs[NRCOMP0_REG], 0x04B8},
		{priv->regs[NDESIGNCAP_REG], 0x2A83},
		{priv->regs[NFULLCFG_REG], 0x0045},
		{priv->regs[NICHGTERM_REG], 0x01E0},
	...
	if ((priv->driver_data == ID_MAX17330) || (priv->driver_data == ID_MAX17332)) {
		...
		if (!(ret < 0)) {
			// Unlock Write Protection
			dev_info(priv->dev, "max17332 rewrite nvm!!\n");
			regmap_write(priv->regmap, priv->regs[COMMSTAT_REG], data & 0xFF06);
			regmap_write(priv->regmap, priv->regs[COMMSTAT_REG], data & 0xFF06);
			...
			if (priv->driver_data == ID_MAX17330) {
				...
				for (reg_ctr = 0; reg_ctr < ARRAY_SIZE(max17332_nvm_defaults); reg_ctr++)
					regmap_write(priv->regmap, (max17332_nvm_defaults[reg_ctr][0] & 0xFF),
...
static struct max1720x_platform_data *max1720x_parse_dt(struct device *dev)
{
	...
	ret = of_property_read_string(np, "cs-name", &pdata->cs_name);
	if (ret < 0)
		pdata->cs_name = "current_selector_master";

	ret = of_property_read_u32(np, "disable-cs", &pdata->disable_cs);
	if (ret < 0)
		pdata->disable_cs = 0;
	...
	ret = of_property_read_u32(np, "cs-gpios", &pdata->gpios);
	if (ret < 0)
		pdata->gpios = 1;
...
static const struct charger_ops max17332_chg_ops = {
	.get_vbus_adc = max17332_get_vbus,
	.get_vbat_adc = max17332_get_vbat,
	.get_cs_current = max17332_get_ibat,
	.set_constant_voltage = max17332_set_cv,
	.set_charging_current = max17332_set_cc,
	.dump_registers = max17332_dump_registers,
	.dump_init_setting = max17332_dump_init_setting,
	.cs_init_setting = max17332_init_setting,
	.is_charging_done = max17332_chg_is_charge_done,
	.check_cs_temp = max17332_check_cs_temp,
	.cs_status_control = max17332_cs_status_control,
	.event = max17332_do_event,
};

static int max17332_register_chgdev(struct max1720x_priv *priv)
{
	priv->chg_prop.alias_name = priv->pdata->cs_name;
	priv->chg_dev = charger_device_register(priv->pdata->cs_name, priv->dev,
									priv, &max17332_chg_ops,
									&priv->chg_prop);
	return priv->chg_dev ? 0 : -EINVAL;
}
...
static char *max17332_psy_supplied_to[] = {
	"mt6363-gauge",  // connect to 6363 gauge
};

static const struct power_supply_desc max17332_cs_desc_master = {
	.name = "max17332-master",
	.type = POWER_SUPPLY_TYPE_UNKNOWN,
	.properties = max17332_cs_props,
	.num_properties = ARRAY_SIZE(max17332_cs_props),
	.get_property = max17332_get_property,
	.set_property = max17332_set_property,
						  ...
						  const struct i2c_device_id *id)
{
	...
	// struct max17332_chip *chip;
	int ret;
	//u32 data;
	unsigned short temp_rec = 0;
	u32 heatlim_val;
	char *name = NULL;

	dev_info(&client->dev, "max17332 start probe: %d\n", id->driver_data);
	...
	if (priv->pdata->disable_cs) {
		dev_info(&client->dev, "cs_disable = %d\n", priv->pdata->disable_cs);
		return -1;
	}
	priv->dev = &client->dev;
	...
	dev_info(&client->dev, "cs_name = %s\n", priv->pdata->cs_name);
	if (!strcmp(priv->pdata->cs_name, "current_selector_master")) {
		psy_cfg.drv_data = priv;
		psy_cfg.supplied_to = max17332_psy_supplied_to;
		psy_cfg.num_supplicants = ARRAY_SIZE(max17332_psy_supplied_to);

		dev_info(&client->dev, "current_selector_master\n");
		temp_rec = client->addr;
		dev_info(&client->dev, "check irq %d, %d\n",
					client->irq, client->addr);
	} else {
		dev_info(&client->dev, "not support max17332\n");
		return -1;
	}

	if (IS_ERR(priv->psy)) {
		ret = PTR_ERR(priv->psy);
		dev_info(&client->dev, "failed to register battery: %d\n", ret);
		goto err_supply;
	}

	dev_info(priv->dev, "Start to reg chgdev\n");
	ret = max17332_register_chgdev(priv);
	if (ret < 0) {
		dev_info(priv->dev, "Failed to reg chgdev(%d)\n", ret);
	...
	if (ret < 0) {
		dev_info(priv->dev, "Failed to create attribute group [%d]\n",
				ret);
		priv->attr_grp = NULL;
		goto err_attr;
	}
...
/* tmp */
	ret = max1720x_regmap_write(priv, priv->regs[NCHGCFG1_REG], 0xFE00);
	if (ret < 0){
		dev_info(priv->dev, "Failed to write HEATLIM\n");
		priv->pdata->disable_cs = 1;
		goto err_unreg_chgdev;
	}
	ret = max1720x_regmap_read(priv, priv->regs[NCHGCFG1_REG], &heatlim_val);
	if (ret < 0){
	...
	dev_info(priv->dev, "HEATLIM = %x\n", heatlim_val);
	priv->psy = power_supply_register(&client->dev,
										  &max17332_cs_desc_master, &psy_cfg);
	name = devm_kasprintf(priv->dev, GFP_KERNEL, "%s",
		"cs_suspend_wakelock");
	priv->cs_wakelock = wakeup_source_register(NULL, name);
	spin_lock_init(&priv->slock);
	init_waitqueue_head(&priv->wait_que);
	priv->polling_interval = 1;
	cs_init_timer(priv);
	priv->cs_thread_task = kthread_run(cs_allowchgB_threadfn, priv, "cs_allowchgB_thread");
	return 0;
