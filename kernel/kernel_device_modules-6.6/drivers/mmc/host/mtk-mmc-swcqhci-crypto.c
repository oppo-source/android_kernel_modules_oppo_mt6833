...
swcq_host_from_crypto_profile(struct blk_crypto_profile *profile)
{
	struct mmc_host *mmc =
...
static int mmc_crypto_keyslot_program(struct blk_crypto_profile *profile,
			const struct blk_crypto_key *key,
			unsigned int slot)

{
	struct swcq_host *cq_host = swcq_host_from_crypto_profile(profile);
	int err = 0;
	u8 data_unit_mask;
...
#if IS_ENABLED(CONFIG_MMC_CRYPTO_LEGACY)
	...
	if (err) {
		dev_info(mmc_dev(cq_host->mmc), "[%s]: write key to crypto_cfg error: %d\n",
			__func__, err);
		return err;
	}
...
static int mmc_crypto_keyslot_evict(struct blk_crypto_profile *profile,
			const struct blk_crypto_key *key,
			unsigned int slot)
{
	struct swcq_host *cq_host = swcq_host_from_crypto_profile(profile);
...
static const struct blk_crypto_ll_ops swcq_crypto_ops = {
...
static int swcq_mmc_init_crypto_spec(struct mmc_host *mmc)
{
	...
	profile->ll_ops = swcq_crypto_ops;
	profile->dev = mmc_dev(mmc);
	...
	profile->modes_supported[1] = 0x1200;

	dev_dbg(dev, "mmc crypto init done 0x%p\n", &(mmc->crypto_profile.ll_ops));
...
int swcq_mmc_init_crypto(struct mmc_host *mmc)
{
	...
	return swcq_mmc_init_crypto_spec(mmc);
}
