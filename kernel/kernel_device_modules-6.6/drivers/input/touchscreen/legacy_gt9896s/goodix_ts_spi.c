...
int gt9896s_mt6877_cfg_flag;
int gt9896s_find_mt6877_touch_node;
...
static int mtk_drm_lcm_info_get(struct gt9896s_ts_board_data *board_data)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,legacy-lcm-workaround");
	if (node) {
		...
		if (chosen_node) {
			...
			if (videolfb_tag) {
				lcm_name_len = strlen((char *)videolfb_tag + (2 + 3)*4);
				offset = 2 + 3 + ((lcm_name_len + 1 + 4) >> 2);
				board_data->lcm_max_x = *((unsigned int *)videolfb_tag + offset) & 0xffff;
				board_data->lcm_max_y = *((unsigned int *)videolfb_tag + offset + 1) & 0xffff;
				gt9896s_mt6877_cfg_flag = 0;
				gt9896s_find_mt6877_touch_node = 1;
				ts_err("find display x & y:width:0x%x, heigh:0x%x",
						board_data->lcm_max_x, board_data->lcm_max_y);
			} else {
				gt9896s_find_mt6877_touch_node = 0;
				ts_err("[touch] videolfb_tag not found");
			}
		} else {
			gt9896s_find_mt6877_touch_node = 0;
			ts_err("touch of_chosen not found");
		}
	} else {
		gt9896s_find_mt6877_touch_node = 0;
		ts_err("common lcm node not found");
	}
	return 0;
}
...
static int gt9896s_spi_probe(struct spi_device *spi)
{
	...
	if (IS_ENABLED(CONFIG_OF) && spi->dev.of_node) {
		...
		r = mtk_drm_lcm_info_get(&ts_device->board_data);
		if (r < 0) {
			ts_info("%s OUT, not get lcm info", __func__);
			return r;
		}
		...
		if (gt9896s_find_mt6877_touch_node == 1) {
			if (ts_device->board_data.lcm_max_x != 0 && ts_device->board_data.lcm_max_y != 0) {
				if (ts_device->board_data.lcm_max_x == 1080
					&& ts_device->board_data.lcm_max_y == 2300) {
					strscpy(panel_config_buf,
						"gt9896s_cfg_6877v01_1080x2300", 30);
					strscpy(panel_firmware_buf,
						"gt9896s_firmware_6877v01_1080x2300", 35);
				} else if (ts_device->board_data.lcm_max_x == 1080
					&& ts_device->board_data.lcm_max_y == 2280) {
					strscpy(panel_config_buf,
						"gt9896s_cfg_6877v01_1080x2280", 30);
					strscpy(panel_firmware_buf,
						"gt9896s_firmware_6877v01_1080x2280", 35);
				} else {
					ts_info("%s, fault firmware!", gt9896s_lcm_buf);
				}
			}
