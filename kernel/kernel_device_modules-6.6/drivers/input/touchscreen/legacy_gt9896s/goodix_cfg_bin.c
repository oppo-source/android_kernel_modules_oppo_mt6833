...
static int gt9896s_read_cfg_bin(struct device *dev, struct gt9896s_cfg_bin *cfg_bin)
{
	...
	} else if (gt9896s_find_mt6877_touch_node == 1) {
		if (gt9896s_mt6877_cfg_flag == 0) {
			strncat(panel_config_buf, ".bin", 4);
			strscpy(cfg_bin_name, panel_config_buf, sizeof(cfg_bin_name));
			gt9896s_mt6877_cfg_flag = 1;
		}
	} else {
