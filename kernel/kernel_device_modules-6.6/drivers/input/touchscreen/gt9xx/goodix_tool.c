...
s32 init_wr_node(struct i2c_client *client)
{
	...
	if (i) {
		...
		dev_dbg(&gt_client->dev, "Apply for memory failed.");
		return FAIL;
	}
	...
	goodix_proc_entry = proc_create(procname, 0660, NULL, &gtp_proc_ops);
	if (!goodix_proc_entry) {
		dev_dbg(&gt_client->dev, "Couldn't create proc entry!");
		return FAIL;
	}
...
static u8 comfirm(void)
{
	...
	for (i = 0; i < cmd_head.times; i++) {
		if (tool_i2c_read(buf, 1) <= 0) {
			dev_dbg(&gt_client->dev, "Read flag data failed!");
			return FAIL;
		}
	...
	if (i >= cmd_head.times) {
		dev_dbg(&gt_client->dev, "Can't get the continue flag!");
		return FAIL;
	}
			  ...
			  size_t len, loff_t *off)
{
	...
	if (1 == cmd_head.wr) {
		...
		if (tool_i2c_write(&cmd_head.data[GTP_ADDR_LENGTH -
				   ...
				   cmd_head.addr_len) <= 0) {
			dev_dbg(&gt_client->dev, "[WRITE]Write data failed!");
			return -EPERM;
		}
			 ...
			 size_t size, loff_t *ppos)
{
	...
	if (cmd_head.wr % 2) {
		...
		if (1 == cmd_head.flag) {
			if (FAIL == comfirm()) {
				dev_dbg(&gt_client->dev, "[READ]Comfirm fail!");
				return -EPERM;
			}
		} else if (2 == cmd_head.flag) {
			/*Need interrupt!*/
		...
		while (data_len > 0) {
			...
			if (tool_i2c_read(cmd_head.data, len) <= 0) {
				dev_dbg(&gt_client->dev, "[READ]Read data failed!");
				return -EPERM;
			}
