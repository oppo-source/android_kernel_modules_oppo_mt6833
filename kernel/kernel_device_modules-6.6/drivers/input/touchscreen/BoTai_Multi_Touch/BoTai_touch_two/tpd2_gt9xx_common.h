...
#ifndef tpd2_CUSTOM_GT9XX_H__
#define tpd2_CUSTOM_GT9XX_H__
...
extern unsigned int tpd2_rst_gpio_number;
extern unsigned int tpd2_int_gpio_number;
extern unsigned int touch_irq;

extern s32 ti941_send_cfg(struct i2c_client *client);
extern void ti941_reset_guitar(struct i2c_client *client, s32 ms);
extern void ti941_int_sync(void);
extern u8 gup_init_update_proc(struct i2c_client *client);
extern u8 gup_init_fw_proc(struct i2c_client *client);
extern s32 ti941_i2c_read(struct i2c_client *client, u8 *buf, s32 len);
extern s32 ti941_i2c_write(struct i2c_client *client, u8 *buf, s32 len);
extern int ti941_i2c_write_bytes(struct i2c_client *client, u16 addr, u8 *txbuf, int len);
extern int ti941_i2c_read_bytes(struct i2c_client *client, u16 addr, u8 *rxbuf, int len);
...
/* #define tpd2_HAVE_BUTTON*/    /*report key as coordinate,Vibration feedback *//*config in  dts file */
/* #define tpd2_WARP_X *//*config in defconfig*/
/* #define tpd2_WARP_Y *//*config in defconfig*/
...
#define tpd2_VELOCITY_CUSTOM_X 15
#define tpd2_VELOCITY_CUSTOM_Y 15
...
#define GT91XX_CONFIG_PROC_FILE     "lcm_config_2"
...
#endif
#define tpd2_KEY_COUNT           				5    				//  4
#define tpd2_KEYS                					{KEY_KATAKANAHIRAGANA, KEY_YEN, KEY_RO, KEY_HANGEUL,KEY_F12}
...
#define tpd2_I2C_NUMBER				  0
#define I2C_MASTER_CLOCK              300
#define MAX_I2C_TRANSFER_SIZE         (MAX_TRANSACTION_LENGTH - GTP_ADDR_LENGTH)
#define tpd2_MAX_RESET_COUNT           3
/* #define tpd2_CALIBRATION_MATRIX        {962,0,0,0,1600,0,0,0} */
#define tpd2_CALIBRATION_MATRIX_ROTATION_NORMAL {0, 2559, 0, -6557, 0, 5238784, 0, 0}
#define tpd2_CALIBRATION_MATRIX_ROTATION_FACTORY {0, 2559, 0, -6557, 0, 5238784, 0, 0}


#define tpd2_RESET_ISSUE_WORKAROUND
#define tpd2_HAVE_CALIBRATION
#define tpd2_NO_GPIO
#define tpd2_RESET_ISSUE_WORKAROUND
//#define CONFIG_MTK_BOOT//hc add 

#ifdef tpd2_WARP_X
#undef tpd2_WARP_X
#define tpd2_WARP_X(x_max, x) (x_max - 1 - x)
#else
#define tpd2_WARP_X(x_max, x) x
#endif

#ifdef tpd2_WARP_Y
#undef tpd2_WARP_Y
#define tpd2_WARP_Y(y_max, y) (y_max - 1 - y)
#else
#define tpd2_WARP_Y(y_max, y) y
#endif
//pr_debug("<<-GTP-INFO->> "fmt"\n", ##arg);
/* Log define */
#define GTP_INFO(fmt, arg...)	do {\
	if (GTP_INFO_ON)\
		pr_debug("<<-TI941-INFO->> "fmt"\n", ##arg);\
} while (0)
#define GTP_ERROR(fmt, arg...)          pr_info("<<-TI941-ERROR->> "fmt"\n", ##arg)
#define GTP_DEBUG(fmt, arg...)	do {\
	if (GTP_DEBUG_ON)\
		pr_debug("<<-TI941-DEBUG->> [%d]"fmt"\n", __LINE__, ##arg);\
} while (0)
#define GTP_DEBUG_ARRAY(array, num)	do {\
	s32 i;\
	u8 *a = array;\
	if (GTP_DEBUG_ARRAY_ON) {\
		pr_debug("<<-TI941-DEBUG-ARRAY->>\n");\
		for (i = 0; i < (num); i++) {\
			pr_debug("%02x   ", (a)[i]);\
			if ((i + 1) % 10 == 0)\
				pr_debug("\n");\
...
#define GTP_DEBUG_FUNC()	do {\
	if (GTP_DEBUG_FUNC_ON)\
		pr_debug("<<-TI941-FUNC->> Func:%s@Line:%d\n", __func__, __LINE__);\
} while (0)
