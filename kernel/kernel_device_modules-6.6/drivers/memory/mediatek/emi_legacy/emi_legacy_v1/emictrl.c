...
static int emictrl_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < emi_cen->emi_cen_cnt; i++)
		pr_info("%s: emi_cen_base[%d] = 0x%lx\n", __func__,
			i, (unsigned long)emi_cen->emi_cen_base[i]);
	for (i = 0; i < emi_cen->emi_chn_cnt; i++)
		pr_info("%s: emi_chn_base[%d] = 0x%lx\n", __func__,
			i, (unsigned long)emi_cen->emi_chn_base[i]);
