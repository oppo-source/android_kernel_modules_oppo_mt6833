...
int pmic_spmi_device_add(struct pmic_core *core, char sid)
{
	if (core == NULL) {
		pr_info("%s(NULL)\n", __func__);
		return -EINVAL;
	...
	if (sid < 0 || sid >= MAX_SPMI_DEVICE_NUM) {
		pr_info("%s():spmi device is invalid\n", __func__);
		return -ERANGE;
	}

	if (hyp_pmic_core[sid] != NULL) {
		pr_info("%s():usid(%d) is added\n", __func__, sid);
		return -EEXIST;
	}
	hyp_pmic_core[sid] = core;

	return 0;
}
EXPORT_SYMBOL_GPL(pmic_spmi_device_add);

struct pmic_core *pmic_spmi_device_find(char sid)
{
	struct pmic_core *core = NULL;

	if (sid < 0 || sid >= MAX_SPMI_DEVICE_NUM) {
		pr_info("%s():spmi device id invalid\n", __func__);
		return core;
	}

	if(hyp_pmic_core[sid] != NULL)
		core = hyp_pmic_core[sid];

	return core;
}
EXPORT_SYMBOL_GPL(pmic_spmi_device_find);
...
static int mtk_spmi_pmic_irq_init(struct pmic_core *core)
{
	...
	core->irq_domain = irq_domain_add_linear(core->dev->of_node,
						 ...
						 core);
	if (!core->irq_domain) {
		dev_info(core->dev, "Could not create IRQ domain\n");
		return -ENODEV;
	}
	...
	if (ret) {
		dev_info(core->dev, "Failed to register IRQ=%d, ret=%d\n",
			core->irq, ret);
		return ret;
	}
...
static int mtk_spmi_pmic_probe(struct spmi_device *sdev)
{
	...
	if (ret || id == 0) {
		dev_info(&sdev->dev, "Failed to read chip id: %d\n", ret);
		return ret;
	}
	...
	if (chip_data->num_pmic_irqs) {
		...
		if (core->irq < 0)
			dev_info(&sdev->dev, "Failed to get irq(%d)\n", core->irq);

		ret = mtk_spmi_pmic_irq_init(core);
		if (ret)
			dev_info(&sdev->dev, "IRQ_init failed(%d)\n", core->irq);
	...
	if (ret) {
		dev_info(&sdev->dev, "Failed to add child devices: %d\n", ret);
		return ret;
	}

	device_init_wakeup(&sdev->dev, true);
	tmp = pmic_spmi_device_add(core, sdev->usid);
	if (tmp)
		pr_info("Failed to add spmi %d devices\n", core->chip_id);
