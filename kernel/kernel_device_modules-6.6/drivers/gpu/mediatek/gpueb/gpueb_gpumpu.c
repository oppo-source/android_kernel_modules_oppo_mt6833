...
int gpueb_gpumpu_init(struct platform_device *pdev)
{
	...
	if (channel_id == -1) {
		gpueb_log_i(GPUEB_TAG, "get channel ID fail!");
		return -1;
	}
	...
	if (ret != IPI_ACTION_DONE) {
		gpueb_log_i(GPUEB_TAG, "ipi register fail!");
		return ret;
	}
	...
	gpueb_log_d(GPUEB_TAG, "cmd=%u, phys_base=%#llx, size=%#llx, sizeof(struct gpumpu_ipi_send_data)=%lu",
