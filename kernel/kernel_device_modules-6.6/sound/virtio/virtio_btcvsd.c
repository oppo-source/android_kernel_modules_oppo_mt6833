...
//
// Origin code: sound/soc/mediatek/common/Mtk-btcvsd.c
// Author: KaiChieh Chuang <kaichieh.chuang@mediatek.com>
//
// Copyright (c) 2023 MediaTek Inc.
// Author: Grand Zhu <Grand.Zhu@mediatek.com>
//
...
static void virtsnd_btcvsd_snd_irq_enable(struct virtsnd_btcvsd_snd *bt)
{
	regmap_update_bits(bt->infra, bt->infra_misc_offset,
			   bt->conn_bt_cvsd_mask, 0);
}

static void virtsnd_btcvsd_snd_irq_disable(struct virtsnd_btcvsd_snd *bt)
{
	regmap_update_bits(bt->infra, bt->infra_misc_offset,
			   bt->conn_bt_cvsd_mask, bt->conn_bt_cvsd_mask);
}

static void virtsnd_btcvsd_snd_set_state(struct virtsnd_btcvsd_snd *bt,
				     struct virtsnd_btcvsd_snd_stream *bt_stream,
				     int state)
{
	...
	if (bt->tx->state == BT_SCO_STATE_IDLE &&
		...
		if (!bt->irq_disabled) {
			...
			virtsnd_btcvsd_snd_irq_disable(bt);
			bt->irq_disabled = 1;
		}
	} else {
		if (bt->irq_disabled) {
			enable_irq(bt->irq_id);
			virtsnd_btcvsd_snd_irq_enable(bt);
			bt->irq_disabled = 0;
		}
	}
	dev_dbg(bt->dev,
...
static int virtsnd_btcvsd_snd_tx_init(struct virtsnd_btcvsd_snd *bt)
{
...
static int virtsnd_btcvsd_snd_rx_init(struct virtsnd_btcvsd_snd *bt)
{
...
static void get_tx_time_stamp(struct virtsnd_btcvsd_snd *bt,
			      struct virtsnd_btcvsd_snd_time_buffer_info *ts)
{
	ts->time_stamp_us = bt->tx->time_stamp;
	ts->data_count_equi_time = bt->tx->buf_data_equivalent_time;
}

static void get_rx_time_stamp(struct virtsnd_btcvsd_snd *bt,
			      struct virtsnd_btcvsd_snd_time_buffer_info *ts)
{
...
static void virtsnd_btcvsd_snd_data_transfer(enum bt_sco_direct dir,
					 u8 *src, u8 *dst,
...
static int btcvsd_tx_clean_buffer(struct virtsnd_btcvsd_snd *bt)
{
	void *dst;
	...
	virtsnd_btcvsd_snd_data_transfer(BT_SCO_DIRECT_ARM2BT,
...
static int virtsnd_btcvsd_read_from_bt(struct virtsnd_btcvsd_snd *bt,
				   ...
				   unsigned int control)
{
	...
	virtsnd_btcvsd_snd_data_transfer(BT_SCO_DIRECT_BT2ARM, src,
...
int virtsnd_btcvsd_write_to_bt(struct virtsnd_btcvsd_snd *bt,
			   enum bt_sco_packet_len packet_type,
			   unsigned int packet_length,
			   unsigned int packet_num,
			   unsigned int blk_size)
...
static irqreturn_t virtsnd_btcvsd_snd_irq_handler(int irq_id, void *dev)
{
	struct virtsnd_btcvsd_snd *bt = dev;
	...
	if (bt->rx->state == BT_SCO_STATE_RUNNING ||
		...
		if (!bt->rx->xrun &&
			...
			virtsnd_btcvsd_read_from_bt(bt,
						packet_type,
						packet_length,
						packet_num,
						buf_cnt_rx,
...
irq_handler_exit:
	*bt->bt_reg_ctl |= BT_CVSD_TX_UNDERFLOW;
	*bt->bt_reg_ctl &= ~BT_CVSD_CLEAR;
	dev_info(bt->dev, "%s(), irq_handler_exit, bt_reg_ctl = 0x%x\n",
		 __func__, *bt->bt_reg_ctl);

	return IRQ_HANDLED;
}

static int wait_for_bt_irq(struct virtsnd_btcvsd_snd *bt,
			   struct virtsnd_btcvsd_snd_stream *bt_stream)
{
...
ssize_t virtsnd_btcvsd_snd_read(struct virtsnd_btcvsd_snd *bt,
			    char __user *buf,
			    size_t count)
{
...
ssize_t virtsnd_btcvsd_snd_write(struct virtsnd_btcvsd_snd *bt,
			     char __user *buf,
			     size_t count)
{
...
static struct virtsnd_btcvsd_snd_stream *get_bt_stream
	(struct virtsnd_btcvsd_snd *bt, struct snd_pcm_substream *substream)
{
...
/* pcm ops */
static const struct snd_pcm_hardware virtsnd_btcvsd_hardware = {
...
static void virtsnd_pcm_init_runtime_hw(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	...
	//hw->rate_min = max(hw->rate_min, (unsigned int)8000);
	hw->rate_min = max_t(unsigned int, hw->rate_min, 8000);
	hw->rate_max = min_not_zero(hw->rate_max, 16000U);
}

static int virtsnd_btcvsd_open(struct snd_pcm_substream *substream)
{
	struct virtsnd_btcvsd_snd *bt = substream->private_data;
	int ret;

	dev_dbg(bt->dev, "%s(), stream %d, substream %p\n",
		__func__, substream->stream, substream);

	snd_soc_set_runtime_hwparams(substream, &virtsnd_btcvsd_hardware);

	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		ret = virtsnd_btcvsd_snd_tx_init(bt);
		bt->tx->substream = substream;
	} else {
		ret = virtsnd_btcvsd_snd_rx_init(bt);
		bt->rx->substream = substream;
	}

	virtsnd_pcm_init_runtime_hw(substream);

	return ret;
}

static int virtsnd_btcvsd_close(struct snd_pcm_substream *substream)
{
	struct virtsnd_btcvsd_snd *bt = substream->private_data;
	struct virtsnd_btcvsd_snd_stream *bt_stream = get_bt_stream(bt, substream);

	dev_dbg(bt->dev, "%s(), stream %d\n", __func__, substream->stream);

	virtsnd_btcvsd_snd_set_state(bt, bt_stream, BT_SCO_STATE_IDLE);
	bt_stream->substream = NULL;
	return 0;
}

static int virtsnd_btcvsd_hw_params(struct snd_pcm_substream *substream,
...
static int virtsnd_btcvsd_hw_free(struct snd_pcm_substream *substream)
{
	struct virtsnd_btcvsd_snd *bt = substream->private_data;
...
static int virtsnd_btcvsd_prepare(struct snd_pcm_substream *substream)
{
	struct virtsnd_btcvsd_snd *bt = substream->private_data;
	struct virtsnd_btcvsd_snd_stream *bt_stream = get_bt_stream(bt, substream);
	...
	virtsnd_btcvsd_snd_set_state(bt, bt_stream, BT_SCO_STATE_RUNNING);
	return 0;
}

static int virtsnd_btcvsd_trigger(struct snd_pcm_substream *substream, int cmd)
{
	struct virtsnd_btcvsd_snd *bt = substream->private_data;
	struct virtsnd_btcvsd_snd_stream *bt_stream = get_bt_stream(bt, substream);
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		...
		virtsnd_btcvsd_snd_set_state(bt, bt_stream, BT_SCO_STATE_ENDING);
		return 0;
	default:
		return -EINVAL;
	}
...
static snd_pcm_uframes_t virtsnd_btcvsd_pointer(struct snd_pcm_substream *substream)
{
	struct virtsnd_btcvsd_snd *bt = substream->private_data;
	struct virtsnd_btcvsd_snd_stream *bt_stream;
...
static int virtsnd_btcvsd_copy(struct snd_pcm_substream *substream,
			       ...
			       void __user *buf, unsigned long count)
{
	...
	/* count: bytes */
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		virtsnd_btcvsd_snd_write(bt, buf, count);
	else
		virtsnd_btcvsd_snd_read(bt, buf, count);

	return 0;
}

static const struct snd_pcm_ops virtsnd_btcvsd_ops = {
	.open = virtsnd_btcvsd_open,
	.close = virtsnd_btcvsd_close,
	.ioctl = snd_pcm_lib_ioctl,
	.hw_params = virtsnd_btcvsd_hw_params,
	.hw_free = virtsnd_btcvsd_hw_free,
	.prepare = virtsnd_btcvsd_prepare,
	.trigger = virtsnd_btcvsd_trigger,
	.pointer = virtsnd_btcvsd_pointer,
	.copy_user = virtsnd_btcvsd_copy,
};
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	struct virtsnd_btcvsd_snd *bt = NULL;
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	bt = snd->btcvsd;
	lpbk_en = bt->tx->state == BT_SCO_STATE_LOOPBACK;
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0]) {
		virtsnd_btcvsd_snd_set_state(bt, bt->tx, BT_SCO_STATE_LOOPBACK);
		virtsnd_btcvsd_snd_set_state(bt, bt->rx, BT_SCO_STATE_LOOPBACK);
	} else {
		virtsnd_btcvsd_snd_set_state(bt, bt->tx, BT_SCO_STATE_RUNNING);
		virtsnd_btcvsd_snd_set_state(bt, bt->rx, BT_SCO_STATE_RUNNING);
	}
	return 0;
}
				   ...
				   unsigned int __user *data, unsigned int size)
{
	...
	struct virtsnd_btcvsd_snd *bt = NULL;
	int ret = 0;
	struct virtsnd_btcvsd_snd_time_buffer_info time_buffer_info_rx;
	...
	if (size > sizeof(struct virtsnd_btcvsd_snd_time_buffer_info))
		return -EINVAL;
	...
	if (copy_to_user(data, &time_buffer_info_rx,
			 sizeof(struct virtsnd_btcvsd_snd_time_buffer_info))) {
				   ...
				   unsigned int __user *data, unsigned int size)
{
	...
	struct virtsnd_btcvsd_snd *bt = NULL;
	int ret = 0;
	struct virtsnd_btcvsd_snd_time_buffer_info time_buffer_info_tx;
...
static const struct snd_kcontrol_new virtsnd_btcvsd_snd_controls[] = {
...
static int virtsnd_btcvsd_kctl_build(struct virtio_snd *snd)
{
	struct virtio_device *vdev = snd->vdev;
	struct device *dev = &vdev->dev;
	int err, i;

	for (i = 0; i < ARRAY_SIZE(virtsnd_btcvsd_snd_controls); i++) {
		const struct snd_kcontrol_new *control = &virtsnd_btcvsd_snd_controls[i];
		struct snd_kcontrol_new template;
		struct snd_kcontrol *kcontrol;

		memcpy(&template, control, sizeof(template));
		template.index = 0;
		kcontrol = snd_ctl_new1(&template, snd);

		err = snd_ctl_add(snd->card, kcontrol);
		if (err < 0) {
			dev_info(dev, "Virtio Sound: Failed to add %s: %d\n",
				control->name, err);
			return err;
		}
	}
...
static int virtsnd_btcvsd_snd_probe(struct virtsnd_btcvsd_snd *btcvsd)
{
	int ret = 0;
	int irq_id;
	u32 offset[5] = {0, 0, 0, 0, 0};
	struct device *dev = btcvsd->dev;
	u32 disable_write_silence = 0;

	/* find btsvsd dts node */
	btcvsd->of_node = of_find_node_by_name(NULL, "mtk-btcvsd-snd");
	if (!btcvsd->of_node)
		return -ENXIO;

	/* init tx/rx */
	btcvsd->rx = devm_kzalloc(dev, sizeof(*btcvsd->rx), GFP_KERNEL);
	if (!btcvsd->rx)
		return -ENOMEM;

	btcvsd->tx = devm_kzalloc(dev, sizeof(*btcvsd->tx), GFP_KERNEL);
	if (!btcvsd->tx)
		return -ENOMEM;
	...
	virtsnd_btcvsd_snd_tx_init(btcvsd);
	virtsnd_btcvsd_snd_rx_init(btcvsd);

	/* irq */
	irq_id = irq_of_parse_and_map(btcvsd->of_node, 0);
	if (irq_id <= 0) {
		dev_info(dev, "%pOFn no irq found\n", btcvsd->of_node);
		return irq_id < 0 ? irq_id : -ENXIO;
	}

	ret = devm_request_irq(dev, irq_id, virtsnd_btcvsd_snd_irq_handler,
	...
	/* iomap */
	btcvsd->bt_pkv_base = of_iomap(btcvsd->of_node, 0);
	if (!btcvsd->bt_pkv_base) {
		dev_info(dev, "iomap bt_pkv_base fail\n");
		return -EIO;
	}

	btcvsd->bt_sram_bank2_base = of_iomap(btcvsd->of_node, 1);
	if (!btcvsd->bt_sram_bank2_base) {
		dev_info(dev, "iomap bt_sram_bank2_base fail\n");
		return -EIO;
	}

	btcvsd->infra = syscon_regmap_lookup_by_phandle(btcvsd->of_node,
							"mediatek,infracfg");
	...
	/* get offset */
	ret = of_property_read_u32_array(btcvsd->of_node, "mediatek,offset",
					 offset,
					 ARRAY_SIZE(offset));
	if (ret) {
	...
	ret = of_property_read_u32(btcvsd->of_node, "disable_write_silence",
	...
	btcvsd->bt_reg_pkt_r = btcvsd->bt_pkv_base + btcvsd->cvsd_mcu_read_offset;
	btcvsd->bt_reg_pkt_w = btcvsd->bt_pkv_base + btcvsd->cvsd_mcu_write_offset;
	btcvsd->bt_reg_ctl = btcvsd->bt_pkv_base + btcvsd->cvsd_packet_indicator;
	btcvsd->disable_write_silence = (u8) disable_write_silence;

	/* init state */
	virtsnd_btcvsd_snd_set_state(btcvsd, btcvsd->tx, BT_SCO_STATE_IDLE);
	virtsnd_btcvsd_snd_set_state(btcvsd, btcvsd->rx, BT_SCO_STATE_IDLE);

	return ret;
}

// static int virtsnd_btcvsd_snd_remove(struct virtsnd_btcvsd_snd *btcvsd)
// {
...
/**
 * virtsnd_btcvsd_build_devs() - Build BT SCO CVSD/MSBC PCM devices.
...
int virtsnd_btcvsd_build_devs(struct virtio_snd *snd)
{
	...
	if (!snd || !snd->card) {
		dev_info(dev, "could not find virtio sound card\n");
		return -EINVAL;
	}
	...
	ret = virtsnd_btcvsd_snd_probe(btcvsd);
	if (ret) {
		dev_info(dev, "could not find BTCVSD device node\n");
		return ret;
	}

	/* create BTCVSD PCM node */
	ret = snd_pcm_new(snd->card, "BTCVSD", 58, 1, 1, &btcvsd->pcm);
	if (ret) {
		dev_info(dev, "snd_pcm_new[%u] failed: %d\n", 58, ret);
		return ret;
	}

	btcvsd->pcm->info_flags = 0;
	btcvsd->pcm->dev_class = SNDRV_PCM_CLASS_GENERIC;
	btcvsd->pcm->dev_subclass = SNDRV_PCM_SUBCLASS_GENERIC_MIX;
	btcvsd->pcm->private_data = btcvsd;
	btcvsd->pcm->nonatomic = true;

	snd_pcm_set_ops(btcvsd->pcm, SNDRV_PCM_STREAM_PLAYBACK, &virtsnd_btcvsd_ops);
	snd_pcm_set_ops(btcvsd->pcm, SNDRV_PCM_STREAM_CAPTURE, &virtsnd_btcvsd_ops);

	ret = virtsnd_btcvsd_kctl_build(snd);
	if (ret == 0) {
		dev_info(dev, "Create BTCVSD device in Virtio Sound successfully!\n");
		snd->btcvsd = btcvsd;
	}
