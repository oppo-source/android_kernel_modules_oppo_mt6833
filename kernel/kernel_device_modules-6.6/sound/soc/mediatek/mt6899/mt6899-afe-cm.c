...
EXPORT_SYMBOL_GPL(mt6899_set_cm_rate);
...
EXPORT_SYMBOL_GPL(mt6899_set_cm_mux);
...
EXPORT_SYMBOL_GPL(mt6899_get_cm_mux);

static int mt6899_convert_cm_ch(unsigned int ch)
{
	return ch - 1;
}
...
int mt6899_set_cm(struct mtk_base_afe *afe, int id,
	       unsigned int update, bool swap, unsigned int ch)
{
	...
	samplerate = mt6899_general_rate_transform_inverse(afe->dev, rate);
...
EXPORT_SYMBOL_GPL(mt6899_set_cm);
...
EXPORT_SYMBOL_GPL(mt6899_enable_cm_bypass);
...
EXPORT_SYMBOL_GPL(mt6899_enable_cm);
...
EXPORT_SYMBOL_GPL(mt6899_is_need_enable_cm);
