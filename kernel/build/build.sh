...
if [ "${SINGLE_MODULE_BUILD}" == "1" ] ; then
  echo "========================================================"
  echo " Building kernel modules"

  set -x
  # TODO:rm -rf vmlinux.symvers
  (cd ${OUT_DIR} && rm -rf vmlinux.symvers && make O=${OUT_DIR} ${TOOL_ARGS} "${MAKE_ARGS[@]}")
  set +x

  if [ -n "${POST_KERNEL_MOD_BUILD_CMDS}" ]; then
    echo "========================================================"
    echo " Running post-kernel-module-build command(s):"
    set -x
    eval ${POST_KERNEL_MOD_BUILD_CMDS}
    set +x
  fi
fi
